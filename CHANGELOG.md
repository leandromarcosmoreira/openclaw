...
## 2026.2.25 (Não Lançado)

### Alterações

- Android/Chat: melhorada a manipulação de entrega de streaming e a qualidade da renderização de markdown na interface nativa de chat do Android, incluindo melhor comportamento do markdown estilo GitHub. (#26079) Obrigado @obviyus.
- Android/Perf de Inicialização: adiado o início do serviço de primeiro plano, movida a inicialização da depuração do WebView para fora da inicialização crítica e adicionados scripts CLI de macrobenchmark de inicialização + perf de baixo ruído para rastreamento determinístico de inicialização a frio. (#26659) Obrigado @obviyus.
- UI/Composição de Chat: adicionado layout empilhado mobile para botões de ação de composição em telas pequenas para melhorar a usabilidade dos controles de envio/sessão. (#11167) Obrigado @junyiz.
- Heartbeat/Config: substituída a alternância de DM de heartbeat por `agents.defaults.heartbeat.directPolicy` (`allow` | `block`; também suportado por agente via `agents.list[].heartbeat.directPolicy`) para semânticas de entrega mais claras.
- Onboarding/Segurança: esclarecidos os avisos de segurança de onboarding de que o OpenClaw é pessoal por padrão (limite de operador confiável único) e configurações compartilhadas/multi-usuário requerem bloqueio/endurecimento explícito.
- Branding/Docs + Superfícies Apple: substituídos o rótulo launchd `bot.molt` restante, bundle-id, subsistema de log e exemplos de comando por `ai.openclaw` em documentos, superfícies de aplicativos iOS, scripts auxiliares e dispositivos de teste CLI.
- Agentes/Config: lembrete aos agentes para chamar `config.schema` antes de edições de configuração ou perguntas de campo de configuração para evitar suposições. Obrigado @thewilloftheshadow.
- Dependências: atualizados os pins de dependência do workspace e o lockfile (Bedrock SDK `3.998.0`, `@mariozechner/pi-*` `0.55.1`, prévia nativa do TypeScript `7.0.0-dev.20260225.1`) mantendo `@buape/carbon` fixado.

### Quebras (Breaking Changes)

- **QUEBRA:** O padrão de entrega direta/DM de heartbeat agora é `allow` novamente. Para manter o comportamento de bloqueio de DM da versão `2026.2.24`, defina `agents.defaults.heartbeat.directPolicy: "block"` (ou substituição por agente).

### Correções

- Agentes/Entrega de subagentes: refatorado o despacho de anúncio de conclusão do subagente em uma máquina de estado explícita de fila/direta/fallback, recuperada a resolução do plugin de canal de saída em estados frios/obsoletos do registro de plugins em caminhos de envio de anúncio/mensagem/gateway, finalizada a contabilidade de limpeza quando o fluxo de anúncio rejeita e tratados envios de Telegram sem `message_id` como falhas de entrega (em vez de IDs `"unknown"` de falso sucesso). (#26867, #25961, #26803, #25069, #26741) Obrigado @SmithLabsLLC e @docaohieu2808.
- Telegram/Webhook: pré-inicializados os bots de webhook, alterado o processamento de webhook para o modo de retorno de chamada JSON e preservadas as leituras completas de carga útil próximas ao limite sob manipuladores atrasados para evitar travamentos de solicitação de webhook e atualizações perdidas. (#26156)
- Slack/Threads de sessão: evitada a herança de sessão pai superdimensionada que inutilizava silenciosamente novas sessões de thread, exibidas falhas de resultado vazio de estouro de contexto incorporadas aos usuários e adicionado `session.parentForkMaxTokens` configurável (padrão `100000`, `0` desativa). (#26912) Obrigado @markshields-tl.
- Cron/Roteamento multi-conta de mensagem: respeitado o `delivery.accountId` explícito para resolução de entrega cron isolada e, quando `message.send` omitir `accountId`, retorna para a conta de canal vinculada do agente remetente em vez de usar o padrão da conta global. (#27015, #26975) Obrigado @lbo728 e @stakeswky.
- Gateway/Raízes de mídia de mensagem: passado o `agentId` através do RPC de `send` do gateway e preferido o `agentId` explícito sobre a resolução de sessão/padrão para que os envios de mídia do workspace do agente não padrão não falhem mais com `LocalMediaAccessError`; adicionada cobertura de regressão para precedência de agente e fallback de agente em branco. (#23249) Obrigado @Sid-Qin.
- Acompanhamentos/Roteamento: quando o roteamento de origem explícito falha, permite o despacho de fallback no mesmo canal (enquanto ainda bloqueia o fallback entre canais) para que as respostas de acompanhamento não sejam perdidas em falhas transitórias do adaptador de origem. (#26109) Obrigado @Sid-Qin.
- Cron/Guarda de anúncio duplicado: rastreada a tentativa de anúncio/entrega direta separadamente do `delivered` confirmado e suprimidos os resumos cron da sessão principal de fallback quando a entrega já foi tentada para evitar envios duplicados ao usuário final em caminhos de confirmação incertos. (#27018)
- LINE/Ciclo de vida: mantido o `startAccount` do LINE pendente até o aborto para que a inicialização do webhook não seja mais lida incorretamente como saída imediata do canal, prevenindo tempestades de loop de reinicialização na inicialização do provedor LINE. (#26528) Obrigado @Sid-Qin.
- Discord/Gateway: capturados e drenados eventos de `error` do gateway em tempo de inicialização antes que os ouvintes de ciclo de vida se conectem para que erros iniciais `Fatal Gateway error: 4014` apareçam como orientação de intenção acionável em vez de travamentos de gateway não capturados. (#23832) Obrigado @theotarr.
- Discord/Texto de entrada: preservado o texto de fallback `title` + `description` do embed no processamento de snapshot de mensagem e encaminhamento para que os títulos do embed não sejam silenciosamente descartados da entrada do agente. (#26946) Obrigado @stakeswky.
- Slack/Fallback de mídia de entrada: entrega de mensagens apenas de arquivo mesmo quando os downloads de mídia do Slack falham, adicionando um fallback de marcador de posição de nome de arquivo, limitando os nomes de fallback ao limite de arquivo de mídia compartilhado e normalizando nomes de arquivos vazios para `file` para que as mensagens com apenas anexos não sejam silenciosamente descartadas. (#25181) Obrigado @justinhuangcode.
- Telegram/Limpeza de prévia: mantidas as prévias de texto finalizadas quando uma mensagem de assistente posterior é apenas mídia (por exemplo, turnos mistos de texto mais voz), pulando o arquivamento de prévia finalizado nos limites da mensagem do assistente, evitando que a limpeza exclua mensagens de texto finais já visíveis. (#27042)
- Telegram/Spoilers Markdown: mantidos pares válidos de `||spoiler||` enquanto deixa delimitadores `||` finais sem correspondência como texto literal, evitando a supressão falsa de spoiler de tudo ou nada. (#26105) Obrigado @Sid-Qin.
- Slack/Canais na lista de permissões: correspondência de IDs de canal sem distinção entre maiúsculas e minúsculas durante a resolução da lista de permissões de canais para que as chaves de configuração minúsculas (por exemplo, `c0abc12345`) correspondam corretamente aos IDs de tempo de execução do Slack (`C0ABC12345`) sob `groupPolicy: "allowlist"`, evitando perdas silenciosas de eventos de canal. (#26878) Obrigado @lbo728.
- Discord/Indicador de digitação: evitados indicadores de digitação travados selando os retornos de chamada de keepalive de digitação do canal após a inatividade/limpeza e garantindo que o despacho do Discord sempre marque a digitação como inativa mesmo se a limpeza do fluxo de prévia falhar. (#26295) Obrigado @ngutman.
- Canais/Indicador de digitação: protegidos os retornos de chamada de início de keepalive de digitação após o fechamento por inatividade/limpeza para que os tiques pós-fechamento não possam disparar novamente indicadores de digitação obsoletos. (#26325) Obrigado @win4r.
- Acompanhamentos/Indicador de digitação: garantido que os turnos de acompanhamento marquem o despacho como inativo em cada caminho de saída (incluindo `NO_REPLY`, cargas úteis vazias e erros de agente) para que a limpeza do keepalive de digitação sempre seja executada e os indicadores de digitação do canal não fiquem travados após acompanhamentos enfileirados/silenciosos. (#26881) Obrigado @codexGW.
- Chamada de voz/Ferramentas TTS: ocultada a ferramenta `tts` quando o provedor de mensagens é `voice`, evitando que as execuções de chamada de voz selecionem o TTS de auto-reprodução e caiam em loops silenciosos sem saída. (#27025)
- Agentes/Ferramentas: normalizados os resultados de ferramenta de plugin não padrão que omitem `content` para que as execuções incorporadas não travem mais com `Cannot read properties of undefined (reading 'filter')` após a conclusão da ferramenta (incluindo `tesseramemo_query`). (#27007)
- Cron/Substituições de modelo: quando o `payload.model` isolado não está mais na lista de permissões, retorna para a seleção de modelo padrão em vez de falhar o trabalho, enquanto ainda retorna erros explícitos para strings de modelo inválidas. (#26717) Obrigado @Youyou972.
- Agentes/Fallback de modelo: mantidas as cadeias de fallback de texto + imagem explícitas acessíveis mesmo quando as listas de permissões `agents.defaults.models` estão presentes, preferido o `agentId` de execução explícito sobre a análise de chave de sessão para resolução de substituição de fallback de acompanhamento (com fallback de chave de sessão), tratados os overrides de fallback de nível de agente conforme configurado na pré-execução do executor incorporado e classificados os erros `model_cooldown` / `cooling down` como `rate_limit` para que o failover continue. (#11972, #24137, #17231)
- Agentes/Fallback de modelo: mantidas as cadeias de fallback do mesmo provedor ativas quando o modelo da sessão difere do primário configurado, inferido o motivo do cooldown do estado do perfil do provedor (em vez de apenas `disabledReason`), mantidos os provedores de fallback sem perfil elegíveis (caminhos env/models.json) e relaxadas apenas as tentativas de fallback de cooldown do mesmo provedor para `rate_limit`. (#23816) obrigado @ramezgaberiel.
- Agentes/Fallback de modelo: continuação da travessia de fallback em erros não reconhecidos quando candidatos permanecem, enquanto ainda lança o erro desconhecido original no último candidato. (#26106) Obrigado @Sid-Qin.
- Modelos/Sondas de autenticação: mapeados motivos de falha de autenticação permanentes (`auth_permanent`, por exemplo, chaves revogadas) no status de autenticação da sonda em vez de `unknown`, para que `openclaw models status --probe` reporte falhas de autenticação acionáveis. (#25754) obrigado @rrenamed.
- Hooks/Metadados de entrada: incluídos `guildId` e `channelName` nos metadados de `message_received` para os caminhos de hook de plugin e internos. (#26115) Obrigado @davidrudduck.
- Discord/Autenticação de componente: avaliadas as interações de componente de guilda com autorizadores de gating de comando para que usuários não autorizados não recebam mais `CommandAuthorized: true` em eventos de modal/botão. (#26119) Obrigado @bmendonca3.
- Segurança/Autenticação de gateway: emparelhamento obrigatório para sessões de identidade de dispositivo de operador autenticadas com autenticação de token compartilhado para que dispositivos não emparelhados não possam se auto-atribuir escopos de operador. Obrigado @tdjackey por reportar.
- Segurança/Autenticação WebSocket de gateway: aplicadas verificações de origem para clientes WebSocket de navegador direto além da UI de Controle/Webchat, aplicada limitação de falha de autenticação de senha para tentativas de loopback de origem do navegador (incluindo localhost) e bloqueado o auto-emparelhamento silencioso para clientes de navegador que não são da UI de Controle para evitar cadeias de força bruta de origem cruzada e tomada de sessão. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @luz-oasis por reportar.
- Segurança/Proxy confiável de gateway: exigida a função `operator` para o desvio de emparelhamento de proxy confiável da UI de Controle para que sessões de `node` não emparelhadas não possam mais se conectar via `client.id=control-ui` e invocar métodos de evento de nó. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Onboarding beta do macOS: removido o login OAuth da Anthropic e o caminho de onboarding `oauth.json` legado que expunha o verificador PKCE via `state` do OAuth; isso impactava apenas o caminho de onboarding beta do macOS. A autenticação de assinatura da Anthropic agora é apenas por setup-token e será enviada na próxima versão do npm (`2026.2.25`). Obrigado @zdi-disclosures por reportar.
- Segurança/Consentimento de arquivo do Microsoft Teams: vinculado o aceite/recusa de upload de `fileConsent/invoke` à conversa de origem antes de consumir uploads pendentes, evitando uploads de arquivos pendentes em conversas cruzadas ou cancelamento via valores de `uploadId` vazados; inclui cobertura de regressão para manipulação de invocação de correspondência/não correspondência. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Gateway: endurecido o tratamento de caminhos de `agents.files` para bloquear alvos de symlink fora do workspace para `agents.files.get`/`agents.files.set`, mantido o suporte para alvos de symlink dentro do workspace e adicionada cobertura de regressão de gateway para fugas bloqueadas e symlinks permitidos dentro do workspace. Obrigado @tdjackey por reportar.
- Segurança/Workspace FS: rejeitados apelidos de arquivo do workspace com link físico (hardlink) nas verificações de limite de `tools.fs.workspaceOnly` e `tools.exec.applyPatch.workspaceOnly` (incluindo guardas de montagem de sandbox) para evitar leitura/escrita fora do workspace via caminhos de hardlink dentro do workspace. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Caminhos temporários do navegador: endurecido o tratamento de caminhos de saída de rastreamento/download contra fugas de raiz de symlink e pai de symlink com verificações de caminho de escrita baseadas em realpath, além de validação segura de diretório temporário de fallback que falha em links de fallback inseguros. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Uploads do navegador: revalidados os caminhos de upload no momento do uso nos fluxos de seletor de arquivos do Playwright e de entrada direta para que caminhos ausentes/vinculados novamente sejam rejeitados antes de `setFiles`, com cobertura de regressão para tratamento rigoroso de caminhos ausentes.
- Segurança/Aprovações de execução: vinculada a correspondência de aprovação `system.run` à identidade exata de argv e preservado o espaço em branco de argv no texto do comando renderizado, evitando a troca de caminhos executáveis com espaços à direita de reutilizar uma aprovação incompatível. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Aprovações de execução: endurecida a execução de `system.run` vinculada à aprovação em hosts de nó rejeitando caminhos `cwd` de symlink e canonicalizando argv executáveis semelhantes a caminhos antes do spawn, bloqueando cadeias de redirecionamento de symlink mutável entre a aprovação e a execução. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Signal: imposta a autorização de DM/grupo antes do enfileiramento de notificação apenas de reação para que remetentes não autorizados não possam mais injetar eventos do sistema de reação do Signal sob `dmPolicy`/`groupPolicy`; as notificações de reação agora exigem verificações de acesso ao canal primeiro. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Reações do Discord: imposta a autorização de política de DM/lista de permissões antes do enfileiramento do sistema de eventos de reação em mensagens diretas; o tratamento de reações do Discord agora também honra a ativação de DM/grupo-DM e o gating de canal `groupPolicy` da guilda para manter a entrada de reação alinhada com o preflight normal de mensagem. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Reações + pins do Slack: portão de enfileiramento de eventos do sistema `reaction_*` e `pin_*` através de autorização de remetente compartilhada para que listas de permissões de usuários de canal e `dmPolicy`/`allowFrom` de DM sejam impostas consistentemente para entrada de não-mensagem, com cobertura de regressão para caminhos de remetente negados/permitidos. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Reações do Telegram: impostas `dmPolicy`/`allowFrom` e autorização de lista de permissões de grupo em eventos `message_reaction` antes de enfileirar eventos de sistema de reação, evitando entrada acionada por reação não autorizada em DMs e grupos; será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Lista de permissões de grupo do Telegram: falha por padrão para autorização de remetente de grupo removendo o fallback de armazenamento de emparelhamento de DM da avaliação da lista de permissões do grupo; o acesso do remetente do grupo agora requer `groupAllowFrom` explícito ou `allowFrom` por grupo/por tópico. (#25988) Obrigado @bmendonca3.
- Segurança/Interações do Slack: imposta a autorização de canal/DM e o vínculo do ator do modal (`private_metadata.userId`) antes de enfileirar eventos do sistema `block_action`/`view_submission`/`view_closed`, com cobertura de regressão para remetentes não autorizados e metadados de ator ausentes/incompatíveis. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @tdjackey por reportar.
- Segurança/Nextcloud Talk: descarte de eventos de webhook assinados repetidos com deduplicação de repetição persistente por conta entre reinicializações e rejeição de origens de backend de webhook inesperadas quando a URL base da conta está configurada. Obrigado @aristorechina por reportar.
- Segurança/Nextcloud Talk: rejeição de tráfego de webhook não assinado antes de leituras de corpo completo, reduzindo a exposição do corpo da solicitação não autenticada, com cobertura de regressão de ordem de autenticação. (#26118) Obrigado @bmendonca3.
- Segurança/Nextcloud Talk: interrupção do tratamento de entradas da loja de emparelhamento DM como remetentes da lista de permissões do grupo, para que a autorização do grupo permaneça limitada às listas de permissões do grupo configuradas. (#26116) Obrigado @bmendonca3.
- Segurança/LINE: limite nas leituras de corpo de webhook não assinadas antes do tratamento de autenticação/assinatura para limitar o processamento de corpo não autenticado. (#26095) Obrigado @bmendonca3.
- Segurança/IRC: manutenção das aprovações da loja de emparelhamento apenas para DM e fora da autorização da lista de permissões do grupo do IRC, com testes de regressão de política para resolução de lista de permissões. (#26112) Obrigado @bmendonca3.
- Segurança/Microsoft Teams: isolada a lista de permissões do grupo e a autorização de comando das entradas da loja de emparelhamento DM para evitar o vazamento de autorização entre contextos. (#26111) Obrigado @bmendonca3.
- Segurança/Guarda SSRF: classificados literais multicast IPv6 (`ff00::/8`) como alvos bloqueados/privado-internos em verificações de IP SSRF compartilhadas, evitando que literais multicast ignorem o preflight do host da URL e a validação de resposta DNS. Isso será enviado na próxima versão do npm (`2026.2.25`). Obrigado @zpbrent por reportar.
- Testes/Estabilidade de baixa memória: desativado o `vmForks` do Vitest por padrão em hosts locais de baixa memória (`<64 GiB`), mantida a paralelização da pista de extensão de baixo perfil em 4 workers e alinhados os testes de agente isolado cron com o uso de `setSessionRuntimeModel` para evitar falhas determinísticas da suíte. (#26324) Obrigado @ngutman.

## 2026.2.24

### Alterações

- Auto-resposta/Atalhos de aborto: expandidas as frases de parada independentes (`stop openclaw`, `stop action`, `stop run`, `stop agent`, `please stop` e variantes relacionadas), aceitação de pontuação final (por exemplo, `STOP OPENCLAW!!!`), adição de palavras-chave de parada multilíngues (incluindo formas ES/FR/ZH/HI/AR/JP/DE/PT/RU) e tratamento de `do not do that` exato como um gatilho de parada, preservando a correspondência independente estrita. (#25103) Obrigado @steipete e @vincentkoc.
- Android/UX do App: enviado um fluxo de integração nativo de quatro etapas, movido o pós-integração para um shell de cinco abas (Conectar, Chat, Voz, Tela, Configurações), adicionada uma tela de modo manual/configuração de conexão completa e superfícies de chat/configurações do Android atualizadas para o novo modelo de navegação.
- Talk/Configuração de gateway: adicionada configuração de Talk independente de provedor com compatibilidade legada e expostos os metadados de configuração ElevenLabs do gateway Talk para superfícies de configuração/status.
- Segurança/Auditoria: adicionado `security.trust_model.multi_user_heuristic` para sinalizar prováveis entradas de usuários compartilhados e esclarecer o modelo de confiança do assistente pessoal, com orientação de endurecimento para configurações multi-usuário intencionais (`sandbox.mode="all"`, FS com escopo de workspace, superfície de ferramenta reduzida, sem identidades pessoais/privadas em tempos de execução compartilhados).
- Dependências: atualizados pacotes de tempo de execução e ferramentas principais em todo o workspace (Bedrock SDK, pi runtime stack, OpenAI, Google auth e oxlint/oxfmt), enquanto `@buape/carbon` é intencionalmente mantido fixado.

### Quebras (Breaking Changes)

- **QUEBRA:** A entrega de Heartbeat agora bloqueia alvos diretos/DM quando a análise de destino identifica um chat direto (por exemplo, `user:<id>`, IDs de chat de usuário do Telegram ou números/JIDs diretos do WhatsApp). As execuções de Heartbeat ainda ocorrem, mas a entrega de mensagens diretas é ignorada e apenas destinos não-DM (por exemplo, alvos de canal/grupo) podem receber mensagens de heartbeat de saída.
- **QUEBRA:** Segurança/Sandbox: bloqueado o modo de junção de namespace Docker `network: "container:<id>"` por padrão para containers de sandbox e sandbox-browser. Para manter esse comportamento intencionalmente, defina `agents.defaults.sandbox.docker.dangerouslyAllowContainerNamespaceJoin: true`. Obrigado @tdjackey por reportar.

### Correções

- Roteamento/Isolamento de sessão: endurecido o roteamento de acompanhamento para que as respostas de origem explícita entre canais nunca retornem ao despachante ativo em caso de falha de rota, preservados os metadados de roteamento de resumo de estouro enfileirados (`channel`/`to`/`thread`) através da drenagem de acompanhamento e preferido o contexto do canal de origem sobre as tags de provedor interno para execuções de acompanhamento incorporadas. Isso evita que o contexto do webchat/control-ui sequestre respostas direcionadas ao Discord em sessões compartilhadas. (#25864) Obrigado @Gamedesigner.
- Segurança/Roteamento: falha por padrão para respostas entre canais de sessão compartilhada vinculando a resolução de alvo de saída aos metadados do canal de origem do turno atual (em vez de fallbacks de rota de sessão obsoletos), e conectados esses campos de origem de turno através de planejadores de entrega de gateway + comando com cobertura de regressão. (#24571) Obrigado @brandonwise.
- Roteamento de Heartbeat: evitado o vazamento/spam de heartbeat no Discord e outros destinos de mensagem direta bloqueando os alvos de entrega de heartbeat de chat direto e mantendo os prompts internos de cron/exec de entrega bloqueada apenas para uso interno. (#25871)
- Padrões/Prompts de Heartbeat: alterado o alvo de entrega de heartbeat implícito de `last` para `none` (opção explícita para entrega externa) e uso de redação de prompt de heartbeat cron/exec apenas interna quando a entrega está desativada para que as verificações de segundo plano não induzam o comportamento de retransmissão voltado ao usuário. (#25871, #24638, #25851)
- Auto-resposta/Enfileiramento de Heartbeat: descarte de execuções de heartbeat quando uma sessão já tem uma execução ativa em vez de enfileirar um acompanhamento obsoleto, evitando ramificações duplicadas de resposta de heartbeat após a drenagem da fila. (#25610, #25606) Obrigado @mcaxtr.
- Cron/Entrega de Heartbeat: interrupção da herança de `lastThreadId` da sessão em cache para resolução de alvo de modo heartbeat, a menos que um tópico/thread seja explicitamente solicitado, para que as entregas de cron e heartbeat no modo de anúncio permaneçam em destinos de nível superior em vez de vazar para threads de conversação ativas. (#25730) Obrigado @markshields-tl.
- Desduplicação de ferramenta de mensagens: tratamento dos metadados do canal de origem como autoritativos para a supressão de `message.send` para o mesmo alvo em execuções proativas (heartbeat/cron/exec-event), incluindo contextos de provedores sintéticos, para que as entradas de transcrição de `delivery-mirror` não causem mais envios duplicados no Telegram. (#25835) Obrigado @jadeathena84-arch.
- Canais/Keepalive de digitação: atualização dos retornos de chamada de digitação do canal em um intervalo de keepalive durante respostas longas e limpeza dos temporizadores de keepalive na inatividade/limpeza em chamadas de despachante core + extensão para que os indicadores de digitação não expirem no meio da inferência. (#25886, #25882) Obrigado @stakeswky.
- Agentes/Fallback de modelo: quando uma execução está atualmente em um modelo de fallback configurado, continua a percorrer a cadeia de fallback configurada em vez de colapsar diretamente para apenas o primário, evitando falhas de beco sem saída quando o primário permanece em cooldown. (#25922, #25912) Obrigado @Taskle.
- Gateway/Modelos: honradas as referências explícitas da lista de permissões `agents.defaults.models` mesmo quando os dados do catálogo de modelos empacotados estão obsoletos, sintetizadas as entradas ausentes da lista de permissões em `models.list` e permitida a seleção de `sessions.patch`/`/model` para essas referências sem erros falsos de `model not allowed`. (#20291) Obrigado @kensipe, @nikolasdehor e @vincentkoc.
- Control UI/Agentes: herança de `agents.defaults.model.fallbacks` na entrada de fallback da Visão Geral quando não existe nenhuma entrada de modelo por agente, preservando as substituições de fallback explícitas por agente (incluindo listas vazias). (#25729, #25710) Obrigado @Suko.
- Automação/Subagente/Confiabilidade Cron: honrado `ANNOUNCE_SKIP` nos fluxos de anúncio direto/conclusão de `sessions_spawn` (sem vazamentos de token visíveis ao usuário), adicionadas tentativas de anúncio direto transitório para indisponibilidade de canal (por exemplo, janelas de reconexão do ouvinte do WhatsApp) e incluído `cron` no perfil da ferramenta `coding` para que `/tools/invoke` possa executar ações cron quando explicitamente permitido pela política do gateway. (#25800, #25656, #25842, #25813, #25822, #25821) Obrigado @astra-fer, @aaajiao, @dwight11232-coder, @kevinWangSheng, @widingmarcus-cyber e @stakeswky.
- Discord/Confiabilidade de voz: restaurada a dependência DAVE de tempo de execução (`@snazzah/davey`), adicionadas opções de entrada DAVE configuráveis (`channels.discord.voice.daveEncryption` e `channels.discord.voice.decryptionFailureTolerance`), limpeza de ouvintes de voz/desmontagem de sessão, proteção contra eventos de conexão obsoletos e gatilho de recuperação de retorno controlado após falhas repetidas de descriptografia para melhorar a estabilidade do STT de entrada sob erros de recebimento DAVE. (#25861, #25372, #24883, #24825, #23890, #23105, #22961, #23421, #23278, #23032)
- Discord/Bloqueio de streaming: restaurada a entrega de resposta com bloqueio de streaming suprimindo apenas as cargas de raciocínio (em vez de todas as cargas de `block`), corrigindo as respostas ausentes do Discord no modo `channels.discord.streaming=block`. (#25839, #25836, #25792) Obrigado @pewallin.
- Discord/Proxy + reações + seletor de modelos: busca de proxy de canal integrada nos downloads de mídia/sticker de entrada, uso de busca de metadados de gateway ciente de proxy para configurações de proxy WSL/corporativo, conexão de `messages.statusReactions.{emojis,timing}` no controle de ciclo de vida de reação do Discord e compactação das chaves `custom_id` do seletor de modelos para permanecer abaixo do limite de 100 caracteres do Discord, mantendo a análise retrocompatível. (#25232, #25507, #25564, #25695) Obrigado @openperf, @chilu18, @Yipsh, @lbo728 e @s1korrrr.
- WhatsApp/Reconexão Web: tratamento do status de fechamento `440` como não-tentável (incluindo valores de status em formato de string), interrupção imediata dos loops de reconexão e emissão de orientação ao operador para revincular após resolver conflitos de sessão. (#25858) Obrigado @markmusson.
- WhatsApp/Segurança de raciocínio: supressão de cargas de saída marcadas como raciocínio e descarte abrupto de cargas de texto que começam com `Reasoning:` antes da entrega ao WhatsApp, evitando que blocos de pensamento ocultos vazem para os usuários finais através de caminhos de mensagem final. (#25804, #25214, #24328)
- Matrix/Recibos de leitura: envio de recibos de leitura assim que as mensagens do Matrix chegam (antes do trabalho do pipeline do manipulador), para que os clientes não mostrem mais estados não lidos/enviados de longa duração enquanto as respostas estão sendo processadas. (#25841, #25840) Obrigado @joshjhall.
- Telegram/Respostas: quando a formatação markdown resulta em HTML vazio (por exemplo, trechos apenas de sintaxe em respostas encadeadas), tenta novamente a entrega com texto simples e falha ruidosamente quando as cargas formatadas e simples estão vazias para evitar estados de entrega falsos. (#25096, #25091) Obrigado @Glucksberg.
- Telegram/Busca de mídia: priorização do IPv4 antes do IPv6 na ordenação de endereços DNS fixados por SSRF para que os downloads de mídia ainda funcionem em hosts com roteamento IPv6 quebrado. (#24295, #23975) Obrigado @Glucksberg.
- Telegram/API de saída: substituído o despachante undici global do Node 22 ao aplicar as decisões de `autoSelectFamily` do Telegram para que as chamadas de `fetch` de saída herdem o fallback IPv4 em vez de permanecerem fixas em configurações de despachante obsoletas. (#25682, #25676) Obrigado @lairtonlelis.
- Onboarding/Telegram: integração de canal principal mantida disponível quando a população do registro de plugin está ausente, retornando aos adaptadores integrados e continuando a configuração do assistente com orientação de recuperação acionável. (#25803) Obrigado @Suko.
- Android/Autenticação de gateway: preservado o estado de autenticação de gateway do Android durante a integração, uso do ID do cliente nativo para sessões de operador, nova tentativa com fallback de token compartilhado após falhas de autenticação de token de dispositivo e evitação de limpeza de tokens em erros de conexão transitórios.
- Slack/Roteamento de DM: tratamento de IDs de canal `D*` como mensagens diretas mesmo quando o Slack envia um `channel_type` incorreto, evitando que o tráfego de DM seja classificado incorretamente como chats de canal/grupo. (#25479) Obrigado @mcaxtr.
- Zalo/Política de grupo: imposta a autorização do remetente para mensagens de grupo com `groupPolicy` + `groupAllowFrom` (com fallback para `allowFrom`), comportamento padrão de grupo em tempo de execução para lista de permissões fechada por padrão e bloqueio de mensagens de grupo não autorizadas (não-comando) antes do despacho. Obrigado @tdjackey por reportar.
- macOS/Entrada de voz: proteção de todos os caminhos de inicialização de entrada de áudio contra microfones padrão ausentes (Voice Wake, Modo Talk, Push-to-Talk, monitor de nível de microfone, testador) para evitar travamentos de inicialização/tempo de execução em Macs sem microfone e falha graciosa até que a entrada esteja disponível. (#25817) Obrigado @sfo2001.
- macOS/Entrada IME: quando o texto marcado está ativo, trata o Return como confirmação de candidato IME primeiro no compositor de sobreposição de voz e no compositor de chat compartilhado para evitar envios acidentais ao compor texto CJK. (#25178) Obrigado @bottotl.
- macOS/Roteamento de voz wake: transcrições de voz wake encaminhadas por padrão para o canal `webchat` (em vez de roteamento `last` ambíguo) para que os prompts de voz locais permaneçam fixos na superfície de chat de controle, a menos que explicitamente substituídos. (#25440) Obrigado @chilu18.
- macOS/Inicialização do gateway: preferência por um binário `openclaw` disponível antes do fallback do tempo de execução pnpm/node ao resolver comandos de gateway local, para que a inicialização local não falhe mais em hosts com descoberta de tempo de execução quebrada. (#25512) Obrigado @chilu18.
- macOS/Barra de menu: interrupção do reuso do delegado injetor para o sub-menu "Custo de uso (30 dias)" para evitar loops de injeção recursiva de sub-menu ao abrir o histórico de custos. (#25341) Obrigado @yingchunbai.
- macOS/Painel WebChat: correção do recorte de cantos arredondados usando mesclagem de efeito visual específica do painel e correspondência da máscara de canto em ambas as camadas de efeito e hospedagem. (#22458) Obrigado @apethree e @agisilaos.
- Windows/Seleção de shell Exec: preferência pela descoberta do PowerShell 7 (`pwsh`) (Program Files, ProgramW6432, PATH) antes de retornar ao Windows PowerShell 5.1, corrigindo falhas de encadeamento de comando `&&` em hosts Windows com PS7 instalado. (#25684, #25638) Obrigado @zerone0x.
- Windows/Verificações de segurança de mídia: alinhada a validação de identidade de arquivo local assíncrona com o comportamento de abertura segura síncrona, tratando estatísticas `dev=0` do win32 como fallbacks de dispositivo desconhecido (mantendo verificações rigorosas de dev quando ambos os lados são diferentes de zero), corrigindo descartes falsos de `Local media path is not safe to read` para anexos locais/TTS/imagens. (#25708, #21989, #25699, #25878) Obrigado @kevinWangSheng.
- iMessage/Segurança de raciocínio: endurecida a supressão de eco do iMessage com correspondência de `messageId` de saída (mais fallback de texto com escopo) e imposição de supressão de carga de raciocínio em caminhos de entrega de saída roteados para evitar que o texto de pensamento oculto seja enviado como mensagens de canal visíveis ao usuário. (#25897, #1649, #25757) Obrigado @rmarr e @Iranb.
- Provedores/OpenRouter/Perfis de autenticação: desvio das janelas de resfriamento/desativação do perfil de autenticação para o OpenRouter, para que falhas do provedor não coloquem mais os perfis do OpenRouter em cooldown local e marcadores de cooldown legados obsoletos sejam ignorados nos caminhos de fallback e seleção de status. (#25892) Obrigado @alexanderatallah por levantar isso e @vincentkoc pela correção.
- Provedores/Google raciocínio: saneadas as cargas `thinkingBudget` negativas inválidas para solicitações do Gemini 3.1, descartando orçamentos `-1` e mapeando o esforço de raciocínio configurado para `thinkingLevel`, evitando cargas de raciocínio malformadas em `google-generative-ai`. (#25900)
- Provedores/SiliconFlow: normalizado `thinking="off"` para `thinking: null` para cargas de modelo `Pro/*` para evitar loops 400 no lado do provedor e tentativas de compactação enganosas. (#25435) Obrigado @Zjianru.
- Modelos/Autenticação Bedrock: normalizados aliases adicionais do provedor Bedrock (`bedrock`, `aws-bedrock`, `aws_bedrock`, `amazon bedrock`) para o canônico `amazon-bedrock`, garantindo que a resolução do modo de autenticação selecione consistentemente o fallback do SDK da AWS. (#25756) Obrigado @fwhite13.
- Modelos/Provedores: preservadas as substituições de `reasoning` explícitas do usuário ao mesclar a configuração do modelo do provedor com os metadados do catálogo integrados, para que `reasoning: false` não seja mais substituída pelos padrões do catálogo. (#25314) Obrigado @lbo728.
- Gateway/Autenticação: permitidas sessões de websocket da UI de Controle autenticadas por proxy confiável para pular o emparelhamento de dispositivos quando a identidade do dispositivo está ausente, evitando falhas falsas de `pairing required` atrás de proxies reversos confiáveis. (#25428) Obrigado @SidQin-cyber.
- CLI/Busca de memória: aceitação de `--query <texto>` para `openclaw memory search` (mantendo o suporte a consultas posicionais) e exibição de um erro claro quando nenhuma forma é fornecida. (#25904, #25857) Obrigado @niceysam e @stakeswky.
- CLI/Doctor: corrigidas dicas de recuperação obsoletas para usar comandos válidos (`openclaw gateway status --deep` e `openclaw configure --section model`). (#24485) Obrigado @chilu18.
- Doctor/Sandbox: quando o modo sandbox está ativado mas o Docker está indisponível, exibe um aviso claro e acionável (incluindo o impacto da falha e a solução) em vez de apenas uma nota leve de "pular verificações". (#25438) Obrigado @mcaxtr.
- Doctor/Plugins: a ativação automática agora resolve os plugins de canais de terceiros pelo ID do plugin no manifesto (não pelo ID do canal), evitando gravações inválidas em `plugins.entries.<channelId>` quando os IDs diferem. (#25275) Obrigado @zerone0x.
- Config/Plugins: tratamento de referências obsoletas removidas do plugin `google-antigravity-auth` como avisos de compatibilidade (não erros de validação rígida) em `plugins.entries`, `plugins.allow`, `plugins.deny` e `plugins.slots.memory`, para que a inicialização não falhe mais após a remoção do antigravity. (#25538, #25862) Obrigado @chilu18.
- Config/Meta: aceitação de carimbos de data/hora numéricos `meta.lastTouchedAt` e coação para strings ISO, preservando a compatibilidade com edições de agentes que gravitam valores de `Date.now()`. (#25491) Obrigado @mcaxtr.
- Contabilidade de uso: análise dos campos `cached_tokens` do Moonshot/Kimi (incluindo `prompt_tokens_details.cached_tokens`) em métricas de uso de leitura de cache normalizadas. (#25436) Obrigado @Elarwei001.
- Agentes/Despacho de ferramentas: espera pelo flush da resposta de bloco antes que a execução da ferramenta comece para que as respostas de bloco em buffer preservem a ordem das mensagens em torno das chamadas de ferramentas. (#25427) Obrigado @SidQin-cyber.
- Agentes/Classificação de faturamento: evitado que textos longos voltados para o assistente/usuário sejam reescritos como falhas de faturamento, enquanto preserva a detecção explícita de `status/code/http 402` para cargas de erro estruturadas sobressalentes. (#25680, #25661) Obrigado @lairtonlelis.
- Sessões/Guarda de resultado de ferramenta: evita a geração de entradas de `toolResult` sintéticas para turnos de assistente que terminaram com `stopReason: "aborted"` ou `"error"`, prevenindo que IDs de uso de ferramenta órfãos acionem erros de validação downstream da API. (#25429) Obrigado @mikaeldiakhate-cell.
- Auto-resposta/Hooks de reset: garantia de que os fluxos nativos `/new` e `/reset` emitam hooks de comando/reset mesmo em caminhos de comando de retorno antecipado, com proteção de desduplicação para evitar a emissão dupla de hooks. (#25459) Obrigado @chilu18.
- Hooks/Gerador de slugs: resolução do modelo de slug da sessão a partir do modelo efetivo do agente (incluindo fallbacks/resolução padrão) em vez de apenas a configuração primária bruta do agente. (#25485) Obrigado @SudeepMalipeddi.
- Sandbox/Testes da ponte FS: adicionada cobertura de regressão para nomes de base com hífen inicial para confirmar que as leituras de arquivo do sandbox resolvem para caminhos absolutos do container (e evitam diagnósticos incorretos de opção de shell para nomes de arquivos hifenizados). (#25891) Obrigado @albertlieyingadrian.
- Sandbox/Ponte FS: construção de scripts de shell de caminho canônico com separadores de nova linha (não junções `; `) para evitar erros de sintaxe `do;` do POSIX `sh` que quebravam as operações de leitura-escrita de arquivo/imagem do sandbox. (#25737, #25824, #25868) Obrigado @DennisGoldfinger e @peteragility.
- Sandbox/Config: preservados `dangerouslyAllowReservedContainerTargets` e `dangerouslyAllowExternalBindSources` durante a resolução da configuração docker do sandbox para que as substituições explícitas de montagem vinculada alcancem a validação em tempo de execução. (#25410) Obrigado @skyer-jian.
- Gateway/Segurança: imposição da autenticação de gateway para o caminho exato da raiz do plugin `/api/channels` (mais descendentes `/api/channels/`), com cobertura de regressão para variantes de query/barra final e caminhos quase idênticos que devem permanecer de propriedade do plugin. (#25753) Obrigado @bmendonca3.
- Aprovações de execução: tratamento de `*` puro na lista de permissões como um verdadeiro curinga para executáveis analisados, incluindo buscas de PATH não resolvidas, para que as listas de permissões globais de adesão funcionem como configuradas. (#25250) Obrigado @widingmarcus-cyber.
- iOS/Assinatura: melhoria do `scripts/ios-team-id.sh` para o Xcode 16+ retornando para perfis de provisionamento gerenciados pelo Xcode, adição de orientação acionável quando uma conta Apple existe mas nenhum Team ID pode ser resolvido e ignorância dos diretórios de saída `xcodebuild` do Xcode (`apps/ios/build`, `apps/shared/OpenClawKit/build`, `Swabble/build`). (#22773) Obrigado @brianleach.
- Control UI/Imagens de chat: abertura das imagens ao clicar através de um auxiliar de abertura segura compartilhado (permitindo apenas esquemas de URL seguros) e abertura de novas abas com isolamento de `opener` para bloquear tabnabbing. (#18685, #25444, #25847) Obrigado @Mariana-Codebase e @shakkernerd.
- Segurança/Execução: saneamento do ambiente de execução do host herdado antes da mesclagem, canonicalização do tratamento do PATH herdado e remoção de chaves perigosas (`LD_*`, `DYLD_*`, `SSLKEYLOGFILE` e vetores de injeção relacionados) de execuções não em sandbox. (#25755) Obrigado @bmendonca3.
- Segurança/Hooks: normalização da classificação de chave de sessão de hook com trim/minúsculas mais dobramento Unicode NFKC (por exemplo, largura total `ＨＯＯＫ：...`) para que o envolvimento de conteúdo externo não possa ser ignorado por casos mistos ou prefixos semelhantes. (#25750) Obrigado @bmendonca3.
- Segurança/Chamada de voz: adicionada detecção de repetição de webhook da Telnyx e canonicalização da codificação da assinatura da chave de repetição (formas equivalentes Base64/Base64URL desduplicam juntas), para que entregas duplicadas de webhook assinadas não disparem novamente efeitos colaterais. (#25832) Obrigado @bmendonca3.
- Segurança/Mídia do Sandbox: restrição das concessões de caminho temporário de mídia do sandbox para raízes temporárias gerenciadas pelo OpenClaw em vez de confiança ampla no `os.tmpdir()` do host, e adição de proteções de saída/canal (lint de caminho temporário + testes de fumaça de raiz de mídia) para evitar regressões nas leituras de anexos de mídia local. Obrigado @tdjackey por reportar.
- Segurança/Mídia do Sandbox: rejeitados aliases de mídia temporária OpenClaw vinculados por hard-link (incluindo cadeias de symlink-para-hardlink) durante a resolução do caminho de mídia do sandbox para evitar leituras de alias de inode fora do sandbox. (#25820) Obrigado @bmendonca3.
- Segurança/Ações de mensagem: imposição de verificações de raiz de mídia local para `sendAttachment` e `setGroupIcon` quando `sandboxRoot` não está definido, evitando que a hidratação de anexos leia arquivos arbitrários do host via caminhos absolutos locais. Obrigado @GCXWLP por reportar.
- Segurança/Telegram: imposição da autorização de DM antes do download/escrita de mídia (incluindo grupos de mídia) e movimento do rastreamento de atividade de entrada do Telegram para depois da autorização de DM, evitando gravações em disco de mídia de entrada não autorizadas disparadas pelo remetente. Obrigado @v8hid por reportar.
- Segurança/Workspace FS: normalizados caminhos prefixados com `@` antes das verificações de limite do workspace (incluindo leitura/escrita/edição apenas no workspace e proteções de caminho de montagem do sandbox), evitando que tentativas de fuga por caminho absoluto ignorem a validação da guarda. Obrigado @tdjackey por reportar.
- Segurança/Synology Chat: comportamento de lista de permissões fechada por padrão para entrada de DM, para que `dmPolicy: "allowlist"` com `allowedUserIds` vazio rejeite todos os remetentes em vez de permitir o despacho não autorizado. (#25827) Obrigado @bmendonca3 pela contribuição e @tdjackey por reportar.
- Segurança/Imagens nativas: imposição de `tools.fs.workspaceOnly` para carregamento automático de imagens de prompt nativas (incluindo refs de histórico), evitando que montagens de sandbox fora do workspace sejam ingeridas implicitamente como entrada de visão. Obrigado @tdjackey por reportar.
- Segurança/Aprovações de execução: vínculo do texto de exibição/aprovação do comando `system.run` ao argv completo quando cargas úteis inline de shell-wrapper carregam valores de argv posicionais, e rejeição de incompatibilidades de `rawCommand` apenas de carga útil para essas formas de transportador de wrapper, evitando a execução de comando oculta sob texto de aprovação enganoso. Obrigado @tdjackey por reportar.
- Segurança/Host acompanhante Exec: encaminhamento do texto de exibição canônico de `system.run` (não apenas trechos de shell de carga útil) para o host de execução do macOS e imposição da consistência de rawCommand/argv lá para transportadores de argv posicional de shell-wrapper e prelúdios modificadores de ambiente, evitando desvios de aprovação/exibição no lado do acompanhante. Obrigado @tdjackey por reportar.
- Segurança/Aprovações de execução: falha por padrão quando o desembrulho transparente do wrapper de despacho excede o limite de profundidade, para que cadeias aninhadas de `/usr/bin/env` não possam ignorar a proteção de aprovação do shell-wrapper no modo `allowlist` + `ask=on-miss`. Obrigado @tdjackey por reportar.
- Segurança/Exec: limites para os diretórios confiáveis de safe-bin padrão para caminhos de sistema imutáveis (`/bin`, `/usr/bin`) e requisito de adesão explícita (`tools.exec.safeBinTrustedDirs`) para caminhos de binários de usuários/gerenciadores de pacotes (por exemplo, Homebrew), adição de descobertas de auditoria de segurança para escolhas arriscadas de diretórios confiáveis, aviso em tempo de execução quando diretórios explicitamente confiáveis permitem escrita por grupo/mundo e adição de dicas do doctor quando `safeBins` configurados se resolvem fora dos diretórios confiáveis. Obrigado @tdjackey por reportar.
- Telegram/Busca de mídia: priorização do IPv4 antes do IPv6 na ordenação de endereços DNS fixados por SSRF para que os downloads de mídia ainda funcionem em hosts com roteamento IPv6 quebrado. (#24295, #23975) Obrigado @Glucksberg.
- Telegram/API de saída: substituído o despachante undici global do Node 22 ao aplicar as decisões de `autoSelectFamily` do Telegram para que as chamadas de `fetch` de saída herdem o fallback IPv4 em vez de permanecerem fixas em configurações de despachante obsoletas. (#25682, #25676) Obrigado @lairtonlelis.
- Agentes/Classificação de faturamento: evitado que textos longos voltados para o assistente/usuário sejam reescritos como falhas de faturamento, enquanto preserva a detecção explícita de `status/code/http 402` para cargas de erro estruturadas sobressalentes. (#25680, #25661) Obrigado @lairtonlelis.
- Telegram/Respostas: quando a formatação markdown resulta em HTML vazio (por exemplo, trechos apenas de sintaxe em respostas encadeadas), tenta novamente a entrega com texto simples e falha ruidosamente quando as cargas formatadas e simples estão vazias para evitar estados de entrega falsos. (#25096, #25091) Obrigado @Glucksberg.
- Sessões/Guarda de resultado de ferramenta: evita a geração de entradas de `toolResult` sintéticas para turnos de assistente que terminaram com `stopReason: "aborted"` ou `"error"`, prevenindo que IDs de uso de ferramenta órfãos acionem erros de validação downstream da API. (#25429) Obrigado @mikaeldiakhate-cell.
- Gateway/Sessões: preservado o `modelProvider` em `sessions.reset` e evitação de prefixos de provedor incorretos para modelos de sessão legados. (#25874) Obrigado @lbo728.
- Contabilidade de uso: análise dos campos `cached_tokens` do Moonshot/Kimi (incluindo `prompt_tokens_details.cached_tokens`) em métricas de uso de leitura de cache normalizadas. (#25436) Obrigado @Elarwei001.
- Doctor/Sandbox: quando o modo sandbox está ativado mas o Docker está indisponível, exibe um aviso claro e acionável (incluindo o impacto da falha e a solução) em vez de apenas uma nota leve de "pular verificações". (#25438) Obrigado @mcaxtr.
- Config/Meta: aceitação de carimbos de data/hora numéricos `meta.lastTouchedAt` e coação para strings ISO, preservando a compatibilidade com edições de agentes que gravam valores de `Date.now()`. (#25491) Obrigado @mcaxtr.
- Auto-resposta/Hooks de reset: garantia de que os fluxos nativos `/new` e `/reset` emitam hooks de comando/reset mesmo em caminhos de comando de retorno antecipado, com proteção de desduplicação para evitar a emissão dupla de hooks. (#25459) Obrigado @chilu18.
- Hooks/Gerador de slugs: resolução do modelo de slug da sessão a partir do modelo efetivo do agente (incluindo fallbacks/resolução padrão) em vez de apenas a configuração primária bruta do agente. (#25485) Obrigado @SudeepMalipeddi.
- Slack/Roteamento de DM: tratamento de IDs de canal `D*` como mensagens diretas mesmo quando o Slack envia um `channel_type` incorreto, evitando que o tráfego de DM seja classificado incorretamente como chats de canal/grupo. (#25479) Obrigado @mcaxtr.
- Modelos/Provedores: preservadas as substituições de `reasoning` explícitas do usuário ao mesclar a configuração do modelo do provedor com os metadados do catálogo integrados, para que `reasoning: false` não seja mais substituída pelos padrões do catálogo. (#25314) Obrigado @lbo728.
...
- CLI/Doctor: corrigidas dicas de recuperação obsoletas para usar comandos válidos (`openclaw gateway status --deep` e `openclaw configure --section model`). (#24485) Obrigado @chilu18.
- CLI/Busca de memória: aceitação de `--query <texto>` para `openclaw memory search` (mantendo o suporte a consultas posicionais) e exibição de um erro claro quando nenhuma forma é fornecida. (#25904, #25857) Obrigado @niceysam e @stakeswky.
- Segurança/Sandbox: canonicalização de caminhos de origem de montagem via realpath de ancestral existente para que os caminhos de pai-symlink + folha-inexistente não possam ignorar as raízes de origem permitidas ou verificações de caminhos bloqueados. Obrigado @tdjackey.

## 2026.2.23

### Alterações

- Provedores/Kilo Gateway: adicionado suporte de primeira classe ao provedor `kilocode` (autenticação, integração, detecção implícita de provedor, padrões de modelo, tratamento de transcrição/cache-ttl e docs), com o modelo padrão `kilocode/anthropic/claude-opus-4.6`. (#20212) Obrigado @jrf0110 e @markijbema.
- Provedores/Vercel AI Gateway: aceitação de referências de modelo abreviadas da Claude (`vercel-ai-gateway/claude-*`) normalizando para IDs de modelo roteados pela Anthropic canônicos. (#23985) Obrigado @sallyom, @markbooch e @vincentkoc.
- Docs/Prompt caching: adicionada uma referência dedicada ao prompt-caching cobrindo `cacheRetention`, precedência de mesclagem de `params` por agente, comportamento Bedrock/OpenRouter e ajuste do cache-ttl + heartbeat. Obrigado @svenssonaxel.
- Gateway/Cabeçalhos de segurança HTTP: adicionado suporte opcional a `gateway.http.securityHeaders.strictTransportSecurity` para emitir `Strict-Transport-Security` para implantações HTTPS diretas, com fiação de tempo de execução, validação, testes e documentos de endurecimento.
- Sessões/Cron: endurecida a manutenção de sessão com `openclaw sessions cleanup`, direcionamento de loja por agente, controles de orçamento de disco (`session.maintenance.maxDiskBytes` / `highWaterBytes`) e comportamento de limpeza de transcrição/arquivo + retenção de log de execução mais seguro. (#24753) obrigado @gumadeiras.
- Ferramentas/web_search: adicionado suporte ao provedor `kimi` (Moonshot) com conexão de esquema de chave/configuração e um fluxo de ferramenta `$web_search` de duas etapas corrigido que ecoa os resultados da ferramenta antes da síntese final, incluindo a extração de citações dos resultados da pesquisa. (#16616, #18822) Obrigado @adshine.
- Compreensão de mídia/Vídeo: adicionado um provedor de vídeo Moonshot nativo e inclusão do Moonshot na detecção automática da chave de vídeo, além da refatoração da execução de vídeo para honrar a precedência de baseUrl+header de `entry/config/provider` (correspondendo ao comportamento de áudio). (#12063) Obrigado @xiaoyaner0201.
- Agentes/Config: suporte para substituições de `params` por agente mescladas em cima dos padrões do modelo (incluindo `cacheRetention`) para que agentes de tráfego misto possam ajustar o comportamento do cache de forma independente. (#17470, #17112) Obrigado @rrenamed.
- Agentes/Bootstrap: as capturas instantâneas do arquivo bootstrap agora são armazenadas em cache por chave de sessão e limpas no reset/exclusão da sessão, reduzindo as invalidações do pool de prompt-cache provenientes de gravações em `AGENTS.md`/`MEMORY.md` durante a sessão. (#22220) Obrigado @anisoptera.

### Quebras de Compatibilidade (Breaking)

- **QUEBRA:** a política de SSRF do navegador agora tem o modo de rede confiável como padrão (`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork=true` quando não definido), e a configuração canônica usa `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` em vez de `browser.ssrfPolicy.allowPrivateNetwork`. `openclaw doctor --fix` migra a chave legada automaticamente.

### Correções (Fixes)

- Segurança/Config: redação de chaves dinâmicas com aparência sensível em capturas instantâneas de `config.get` (por exemplo, `env.*` e `skills.entries.*.env.*`) e preservação do comportamento de restauração de ida e volta para esses sentinelas redigidos. Obrigado @merc1305.
- Testes/Vitest: escalonamento dos padrões de trabalhadores paralelos locais por memória do host, manutenção do gateway serial por padrão em hosts com pouca memória e documentação de um comando de fallback de baixo perfil para execuções de land/gate com restrição de memória para evitar OOMs locais. (#24719) Obrigado @ngutman.
- WhatsApp/Política de grupo: correção da filtragem do remetente `groupAllowFrom` quando `groupPolicy: "allowlist"` está definido sem `groups` explícitos — anteriormente todas as mensagens de grupo eram bloqueadas mesmo para remetentes na lista de permissões. (#24670)
- Agentes/Poda de contexto: elegibilidade de `cache-ttl` estendida para os provedores Moonshot/Kimi e ZAI/GLM (incluindo referências de modelo do OpenRouter), para que `contextPruning.mode: "cache-ttl"` não seja mais silenciosamente ignorado para essas sessões. (#24497) Obrigado @lailoo.
- Doctor/Memória: consulta da prontidão de incorporação de memória do agente padrão no lado do gateway durante o `openclaw doctor` (em vez de inferir a partir da integridade genérica do gateway) e aviso quando o probe de memória do gateway está indisponível ou não está pronto, mantendo a orientação de remediação do `openclaw configure`. (#22327) Obrigado @therk.
- Sessões/Loja: canonicalização das chaves de sessão de caixa mista de entrada para metadados e atualizações de rota, e migração das entradas de variantes de caixa legadas para uma única chave minúscula para evitar sessões duplicadas e histórico ausente na TUI/WebUI. (#9561) Obrigado @hillghost86.
- Telegram/Reações: erro de ação de reação com falha suave (política/token/emoji/API), aceitação de `message_id` em snake_case e retorno ao contexto do ID de mensagem de entrada quando o `messageId` explícito é omitido para que as reações de DM permaneçam estáveis sem loops de regeneração. (#20236, #21001) Obrigado @PeterShanxin e @vincentkoc.
- Telegram/Polling: escopo dos deslocamentos de polling persistidos para a identidade do bot e reuso de um caminho único aguardado de parada do executor no abort/retry, evitando o sangramento de deslocamento entre tokens e pollers sobrepostos durante a recuperação de reinicialização/erro. (#10850, #11347) Obrigado @talhaorak, @anooprdawar e @vincentkoc.
- Telegram/Raciocínio: quando `/reasoning off` está ativo, supressão dos segmentos de entrega apenas de raciocínio e bloqueio do reenvio de fallback bruto de texto `Reasoning:`/`<think>` suprimido, evitando o vazamento de raciocínio interno em sessões legadas enquanto preserva a entrega da resposta. (#24626, #24518)
- Agentes/Raciocínio: quando o pensamento padrão do modelo está ativo (por exemplo, `thinking=low`), manter o auto-raciocínio desativado a menos que explicitamente ativado, evitando o vazamento do bloco de pensamento `Reasoning:` em respostas de canal. (#24335, #24290) Obrigado @Kay-051.
- Agentes/Raciocínio: evitada a classificação de erros de raciocínio obrigatório do provedor como estouros de contexto para que essas falhas não disparem mais a recuperação de estouro no estilo de compactação. (#24593) Obrigado @vincentkoc.
- Agentes/Modelos: codificada a entrada de limite de configuração `agents.defaults.model` / `agents.defaults.imageModel` como `string | {primary,fallbacks}`, divisão da resolução de modelo explícita vs efetiva e correção da atribuição de origem de `models status --agent` para que os agentes herdados por padrão sejam rotulados como `defaults`, enquanto a seleção em tempo de execução ainda honra o fallback de padrões. (#24210) Obrigado @bianbiandashen.
- Agentes/Compactação: passar `agentDir` nas execuções do comando manual `/compact` para que a resolução de perfil/autenticação da compactação permaneça com escopo para o agente ativo. (#24133) Obrigado @Glucksberg.
- Agentes/Compactação: passar os metadados do modelo através do tempo de execução embutido para que a sumarização de salvaguarda possa ser executada quando `ctx.model` estiver indisponível, evitando sumarizações de fallback repetidas do tipo `"Summary unavailable due to context limits"`. (#3479) Obrigado @battman21, @hanxiao e @vincentkoc.
- Agentes/Compactação: cancelar a compactação de salvaguarda quando a geração do resumo não puder ser executada (chave de API/modelo ausente ou falha na sumarização), preservando o histórico em vez de truncar para o texto de fallback `"Summary unavailable"`. (#10711) Obrigado @DukeDeSouth e @vincentkoc.
- Agentes/Ferramentas: fazer com que `session_status` leia o uso derivado da transcrição no meio do turno e leia o final dos logs da sessão para relatórios de contexto cientes de cache sem varreduras completas do log. (#22387) Obrigado @1ucian.
- Agentes/Estouro: detecção de formas adicionais de erro de estouro de contexto do provedor (incluindo variantes de `input length` + `max_tokens` que excedem o contexto) para que as falhas sejam roteadas através de caminhos de compactação/recuperação em vez de vazar erros brutos do provedor para os usuários. (#9951) Obrigado @echoVic e @Glucksberg.
- Agentes/Estouro: adicionada detecção de padrão de estouro de contexto em chinês em `isContextOverflowError` para que erros localizados do provedor sejam roteados através dos caminhos de recuperação de estouro. (#22855) Obrigado @Clawborn.
- Agentes/Failover: tratar erros HTTP 502/503/504 como tempos de espera transitórios elegíveis para failover para que as cadeias de fallback possam trocar de provedores/modelos durante interrupções upstream em vez de tentar novamente o mesmo alvo com falha. (#20999) Obrigado @taw0002 e @vincentkoc.
- Auto-resposta/Metadados de entrada: esconder `message_id`/`message_id_full` de chat direto e metadados do remetente apenas a partir do tipo de chat normalizado (não de sentinelas de ID de remetente), preservando a visibilidade dos metadados de grupo e prevenindo a classificação de modo direto falsificada por ID de remetente. (#24373) Obrigado @jd316.
- Auto-resposta/Metadados de entrada: mover os `flags` de entrada dinâmicos (resposta/encaminhamento/thread/histórico) dos metadados do sistema para as informações de conversação do contexto do usuário, prevenindo a invalidação do prompt-cache turno a turno por alternância de flags. (#21785) Obrigado @aidiffuser.
- Auto-resposta/Sessões: remover os rótulos de chave de autenticação das mensagens de confirmação de `/new` e `/reset` para que os avisos de reset de sessão nunca exponham prefixos de chaves de API ou rótulos de chaves de ambiente na saída do chat. (#24384, #24409) Obrigado @Clawborn.
- Slack/Política de grupo: movido o padrão de `groupPolicy` da conta Slack para os padrões do esquema de nível de provedor para que as configurações de várias contas herdem o `channels.slack.groupPolicy` de nível superior em vez de substituir silenciosamente a herança por `allowlist` por conta. (#17579) Obrigado @ZetiMente.
- Provedores/Anthropic: pulada a injeção beta `context-1m-*` para tokens de assinatura/OAuth (`sk-ant-oat-*`) enquanto preservava os betas exigidos pelo OAuth, evitando falhas de autenticação 401 da Anthropic quando `params.context1m` está ativado. (#10647, #20354) Obrigado @ClumsyWizardHands e @dcruver.
- Provedores/DashScope: marcados os endpoints `openai-completions` compatíveis com DashScope como `supportsDeveloperRole=false` para que o OpenClaw envie a função `system` em vez da função `developer` não suportada nas APIs Qwen/DashScope. (#19130) Obrigado @Putzhuawa e @vincentkoc.
- Provedores/Bedrock: desativada a retenção de prompt-cache para modelos Bedrock que não sejam Anthropic para que as requisições Nova/Mistral não enviem metadados de cache não suportados. (#20866) Obrigado @pierreeurope.
- Provedores/Bedrock: aplicados os padrões de `cacheRetention` do Anthropic-Claude e pass-through de tempo de execução para referências de modelo `amazon-bedrock/*anthropic.claude*`, mantendo os modelos Bedrock que não sejam Anthropic excluídos. (#22303) Obrigado @snese.
- Provedores/OpenRouter: remoção do `reasoning_effort` conflitante de nível superior ao injetar o `reasoning.effort` aninhado, evitando falhas de validação de carga 400 do OpenRouter para modelos de raciocínio. (#24120) obrigado @tenequm.
- Provedores/Groq: evitada a classificação de erros de limite TPM do Groq como estouro de contexto para que os caminhos de estrangulamento não disparem mais a lógica de recuperação de estouro. (#16176) Obrigado @dddabtc.
- Gateway/WS: fechamento de inundações de solicitações repetidas pós-handshake `unauthorized role:*` por conexão e amostragem de logs de rejeição duplicados, evitando que um único cliente com mau comportamento degrade a capacidade de resposta do gateway. (#20168) Obrigado @acy103, @vibecodooor e @vincentkoc.
- Gateway/Restart: tratamento dos PIDs dos ouvintes filhos como de propriedade do PID do tempo de execução do serviço durante as verificações de integridade de reinicialização para evitar mortes falsas de processos obsoletos e tempos de espera de reinicialização no launchd/systemd. (#24696) Obrigado @gumadeiras.
- Config/Write: aplicação de `unsetPaths` com atualizações de cópia de caminho imutáveis para que as gravações de configuração nunca mutem os objetos fornecidos pelo chamador, e endurecimento da travessia de caminho de `openclaw config get/set/unset` rejeitando segmentos de chave de protótipo e travessia de propriedade herdada. (#24134) obrigado @frankekn.
- Canais/WhatsApp: aceitação de `channels.whatsapp.enabled` na validação de configuração para corresponder ao comportamento de auto-ativação de canal integrado, evitando falhas de `Unrecognized key: "enabled"` durante a configuração do canal. (#24263)
- Segurança/Exec: detectar comandos ofuscados antes de decisões de lista de permissões de exec e exigir aprovação explícita para padrões de ofuscação. (#8592) Obrigado @CornBrother0x e @vincentkoc.
- Segurança/ACP: endurecimento da auto-aprovação de permissões do cliente ACP para exigir IDs de ferramentas principais confiáveis, ignorar dicas `toolCall.kind` não confiáveis e restringir a auto-aprovação de `read` ao diretório de trabalho ativo para que nomes de ferramentas desconhecidos e leituras de arquivos fora do escopo sempre solicitem permissão. Obrigado @nedlir por reportar.
- Segurança/Skills: escape de valores de prompt, nome de arquivo e caminho de saída controlados pelo usuário na geração da galeria HTML do `openai-image-gen` para evitar XSS persistente na saída `index.html` gerada. (#12538) Obrigado @CornBrother0x.
- Segurança/Skills: endurecimento do empacotamento do `skill-creator` pulando entradas de symlink e rejeitando arquivos cujos caminhos resolvidos escapem da raiz da skill selecionada. (#24260, #16959) Obrigado @CornBrother0x e @vincentkoc.
- Segurança/OTEL: redação de valores sensíveis (chaves de API, tokens, campos de credenciais) dos corpos de log diagnostics-otel, atributos de log e campos de span de erro/razão antes da exportação OTLP. (#12542) Obrigado @brandonwise.
- Segurança/CI: adicionada cobertura de hook de segurança pré-commit para detecção de chave privada e auditoria de dependência de produção, e imposição dessas verificações no CI junto com a varredura básica de segredos. Obrigado @vincentkoc.
- Skills/Python: endurecimento do empacotamento de script de skill e casos de borda de validação (incluindo saídas `.skill` auto-incluídas, análise de frontmatter CRLF, validação estrita de `--days` e carregamento de arquivo de imagem mais seguro), com cobertura expandida de regressão Python. Obrigado @vincentkoc.
- Skills/Python: adicionado linting de CI + pré-commit (`ruff`) e cobertura de descoberta pytest para scripts/testes Python sob `skills/`, incluindo a execução de teste de pacote a partir da raiz do repositório. Obrigado @vincentkoc.

## 2026.2.22

### Alterações

- Control UI/Agentes: painel de Ferramentas agora é orientado por dados a partir do `tools.catalog` em tempo de execução, adicionados rótulos de procedência por ferramenta (`core` / `plugin:<id>` + marcador opcional) e mantida uma lista estática de fallback quando o catálogo em tempo de execução está indisponível.
- Busca na Web/Gemini: adicionado suporte ao provedor Gemini aterrado com detecção automática de provedor e atualizações de configuração/documentação. (#13075, #13074) Obrigado @akoscz.
- Control UI/Cron: adicionada paridade total de edição de cron na web (incluindo clonagem e texto de ajuda/validação mais rico), além do histórico de execução de todos os jobs com controles de paginação/busca/ordenação/multi-filtro e layout de página de cron melhorado para fluxos de agendamento e triagem de falhas mais limpos.
- Provedor/Mistral: adicionado suporte para o provedor Mistral, incluindo incorporações de memória e suporte a voz. (#23845) Obrigado @vincentkoc.
- Atualização/Core: adicionado um atualizador automático integrado opcional para instalações de pacotes (`update.auto.*`), desativado por padrão, com atraso+jitter de implantação estável e cadência horária em beta.
- CLI/Update: adicionado `openclaw update --dry-run` para visualizar ações de canal/tag/alvo/reinicialização sem mutar a configuração, instalar, sincronizar plugins ou reiniciar.
- Config/UI: adicionado filtro de configurações ciente de tags e ampliação dos rótulos de configuração/texto de ajuda para que os campos sejam mais fáceis de descobrir e entender na tela de configuração do painel.
- Canais/Synology Chat: adicionado um plugin de canal Synology Chat nativo com ingresso de webhook, roteamento de mensagens diretas, suporte a envio/mídia de saída, configuração por conta e controles de política de DM. (#23012)
- iOS/Talk: pré-busca de segmentos TTS e supressão de erros esperados de cancelamento de fala para uma reprodução de fala mais suave. (#22833) Obrigado @ngutman.
- Memória/FTS: adicionada filtragem de stop-words em espanhol e português para expansão de consulta no modo de busca apenas FTS, melhorando a recuperação conversacional para ambos os idiomas. Obrigado @vincentkoc.
- Memória/FTS: adicionada tokenização de expansão de consulta ciente de japonês e filtragem de stop-words (incluindo termos de escrita mista como ASCII + katakana) para o modo de busca apenas FTS. Obrigado @vincentkoc.
- Memória/FTS: adicionada filtragem de stop-words em coreano e extração de palavras-chave ciente de partículas (incluindo radicais mistos coreano/inglês) para expansão de consulta no modo de busca apenas FTS. (#18899) Obrigado @ruypang.
- Memória/FTS: adicionada filtragem de stop-words em árabe para expansão de consulta no modo de busca apenas FTS para reduzir o preenchimento conversacional em buscas de memória em árabe. Obrigado @vincentkoc.
- Discord/Lista de permissões: canonicalização de nomes de lista de permissões do Discord resolvidos para IDs e divisão do fluxo de resolução para um comportamento de falha fechada mais claro.
- Canais/Config: unificação do tratamento de configuração de streaming de visualização de canal com um resolvedor compartilhado e caminho de migração canônico.
- Gateway/Auth: unificação da precedência da fonte de credenciais de call/probe/status/auth em auxiliares de resolvedor compartilhados, com cobertura de paridade orientada por tabela em todos os pontos de entrada do gateway.
- Gateway/Auth: refatoração da resolução de credenciais do gateway e caminhos de handshake de autenticação de websocket para usar contextos de autenticação tipados compartilhados, incluindo suporte explícito a `auth.deviceToken` em quadros de conexão e testes.
- Skills: remoção da skill `food-order` agrupada deste repositório; gerencie/instale-a a partir do ClawHub em vez disso.
- Docs/Subagentes: orientação de sessão vinculada a threads agora é prioritária para o canal em vez de específica do Discord, e listagem explícita dos canais que suportam threads. (#23589) Obrigado @osolmaz.

### Quebras de Compatibilidade (Breaking)

- **QUEBRA:** removido o suporte ao provedor Google Antigravity e o plugin `google-antigravity-auth` agrupado. Configurações existentes de modelo/perfil `google-antigravity/*` não funcionam mais; migre para `google-gemini-cli` ou outros provedores suportados.
- **QUEBRA:** as respostas de falha de ferramenta agora ocultam detalhes de erros brutos por padrão. O OpenClaw ainda envia um resumo da falha, mas os sufixos de erro detalhados (por exemplo, mensagens do provedor/tempo de execução e fragmentos de caminhos locais) agora exigem `/verbose on` ou `/verbose full`.
- **QUEBRA:** a integração local da CLI agora define `session.dmScope` como `per-channel-peer` por padrão para configurações de escopo de DM novas/implícitas. Se você depende da continuidade de DM compartilhada entre remetentes, defina explicitamente `session.dmScope` como `main`. (#23468) Obrigado @bmendonca3.
- **QUEBRA:** unificação da configuração de streaming de visualização de canal para `channels.<canal>.streaming` com valores enum `off | partial | block | progress`, e movimento da alternância de stream nativo do Slack para `channels.slack.nativeStreaming`. Chaves legadas (`streamMode`, booleano do Slack `streaming`) ainda são lidas e migradas pelo `openclaw doctor --fix`, mas as configurações/documentações salvas canônicas agora usam os nomes unificados.
- **QUEBRA:** remoção da assinatura de autenticação de dispositivo legada do Gateway `v1`. Os clientes de autenticação de dispositivo agora devem assinar cargas úteis `v2` com o nonce `connect.challenge` por conexão e enviar `device.nonce`; conexões sem nonce são rejeitadas.

### Correções (Fixes)

- Sessões/Resiliência: ignorar metadados de `sessionFile` persistidos inválidos e recorrer ao caminho de transcrição seguro derivado em vez de abortar a resolução da sessão para manipuladores e ferramentas. (#16061) Obrigado @haoyifan e @vincentkoc.
- Sessões/Caminhos: resolução de aliases de diretórios de estado com symlink durante a validação do caminho de transcrição, preservando a compatibilidade segura entre agentes/raízes de estado para caminhos `agents/<id>/sessions/**` válidos. (#18593) Obrigado @EpaL e @vincentkoc.
- Agentes/Compactação: contar auto-compactações apenas após um `auto_compaction_end` de não-tentativa, mantendo o `compactionCount` da sessão alinhado com as compactações concluídas.
- Segurança/CLI: redação de valores sensíveis na saída do `openclaw config get` antes de imprimir os caminhos de configuração, prevenindo vazamento de credenciais na saída/histórico do terminal. (#13683) Obrigado @SleuthCo.
- Agentes/Moonshot: forçar `supportsDeveloperRole=false` para modelos `openai-completions` compatíveis com Moonshot (provedor `moonshot` e URLs de base Moonshot), para que as execuções iniciais não enviem mais funções `developer` não suportadas que disparam erros `ROLE_UNSPECIFIED`. (#21060, #22194) Obrigado @ShengFuC.
- Agentes/Kimi: classificar falhas de `Your request exceeded model token limit` do Moonshot como estouros de contexto para que a auto-compactação e a recuperação de estouro voltada para o usuário disparem corretamente em vez de exibir erros brutos de solicitação inválida. (#9562) Obrigado @danilofalcao.
- Provedores/Moonshot: marcar o Kimi K2.5 como capaz de imagem em definições de modelo implícitas + integração, e atualizar campos de capacidade de provedor explícitos obsoletos (`input`/`reasoning`/limites de contexto) a partir de catálogos implícitos para que as configurações existentes peguem o suporte de visão Moonshot sem reescritas manuais de modelos. (#13135, #4459) Obrigado @manikv12.
- Agentes/Transcrição: habilitação da unificação de turnos consecutivos de usuários para provedores `openai-completions` estritamente não-OpenAI (por exemplo, Moonshot/Kimi), reduzindo as falhas de ordenação de `roles must alternate` em endpoints compatíveis com OpenAI enquanto preserva o comportamento atual do OpenRouter/Opencode. (#7693)
- Instalação/Discord Voice: tornar `@discordjs/opus` uma dependência opcional para que a instalação/atualização do `openclaw` não falhe mais abruptamente quando as compilações nativas do Opus falham, mantendo o `opusscript` como o decodificador de fallback em tempo de execução para fluxos de voz do Discord. (#23737, #23733, #23703) Obrigado @jeadland, @Sheetaa e @Breakyman.
- Docker/Setup: pré-criação de `$OPENCLAW_CONFIG_DIR/identity` durante o `docker-setup.sh` para que os comandos da CLI que precisam da identidade do dispositivo (por exemplo, `devices list`) evitem falhas de `EACCES ... /home/node/.openclaw/identity` em montagens de vinculação restritivas. (#23948) Obrigado @ackson-beep.
- Exec/Background: interrupção da aplicação do timeout de exec padrão em sessões em segundo plano (`background: true` ou `yieldMs` explícito) quando nenhum timeout explícito está definido, para que os jobs em segundo plano de longa duração não sejam mais terminados no limite do timeout padrão. (#23303)
- Slack/Threading: sessões: manutenção da ramificação da sessão pai e contexto do histórico de threads ativos após o primeiro turno, removendo as travas de apenas primeiro turno na inicialização da sessão, busca de histórico de threads e injeção de contexto de prompt de resposta. (#23843, #23090) Obrigado @vincentkoc e @Taskle.
- Slack/Threading: respeitar `replyToMode` quando o Slack popula automaticamente o `thread_ts` de nível superior e ignorar as tags de diretiva `replyToId` inline quando `replyToMode` está `off`, para que a forçagem de thread permaneça desativada a menos que explicitamente configurada. (#23839, #23320, #23513) Obrigado @vincentkoc e @dorukardahan.
- Slack/Extensão: encaminhamento do `threadId` de `message read` para `readMessages` e uso do `threadId` do contexto de entrega como fallback de `thread_ts` de saída para que as respostas/leituras de extensão permaneçam na thread do Slack correta. (#22216, #22485, #23836) Obrigado @vincentkoc, @lan17 e @dorukardahan.
- Slack/Upload: resolução de IDs de usuário puros (prefixo U) para IDs de canal de DM através de `conversations.open` antes de chamar `files.uploadV2`, que rejeita IDs de não-canal. O `chat.postMessage` tolera IDs de usuário diretamente, mas `files.uploadV2` → `completeUploadExternal` valida o `channel_id` contra `^[CGDZ][A-Z0-9]{8,}$`, causando `invalid_arguments` quando agentes respondem com mídia a conversas de DM.
- Webchat/Chat: aplicação de mensagens de carga `final` do assistente diretamente ao estado do chat para que os turnos enviados sejam renderizados sem esperar por um ciclo completo de atualização de histórico. (#14928) Obrigado @BradGroux.
- Webchat/Chat: para eventos finais fora de banda (por exemplo, execuções laterais de chamada de ferramenta), anexar as cargas úteis finais do assistente fornecidas diretamente em vez de forçar um reset de histórico transitório. (#11139) Obrigado @AkshayNavle.
- Webchat/Performance: recarregar `chat.history` após eventos finais apenas quando a carga final carece de uma mensagem de assistente renderizável, evitando recarregamentos de histórico completo caros em turnos normais. (#20588) Obrigado @amzzzzzzz.
- Webchat/Sessões: preservar os metadados de roteamento de sessão externa quando turnos internos de `chat.send` são executados sob `webchat`, para que sessões explicitamente vinculadas a canais (por exemplo, Telegram) não sejam mais reescritas para `webchat` e causem erro de roteamento na entrega subsequente. (#23258) Obrigado @binary64.
- Webchat/Sessões: preservar o `label` da sessão existente em rolagens de `/new` e `/reset` para que as sessões redefinidas permaneçam localizáveis nas listas de histórico de sessão. (#23755) Obrigado @ThunderStormer.
- Gateway/Chat UI: remover as tags de diretiva de resposta/áudio inline de transmissões webchat finais não-streaming (incluindo `chat.inject`) enquanto preserva o conteúdo da mensagem de string vazia quando as tags são toda a resposta. (#23298) Obrigado @SidQin-cyber.
- Chat/UI: remover as tags de diretiva de resposta/áudio inline (`[[reply_to_current]]`, `[[reply_to:<id>]]`, `[[audio_as_voice]]`) do histórico de chat exibido, saída de evento de chat ao vivo e trechos de visualização de sessão para que as tags de controle não vazem mais nas superfícies visíveis ao usuário.
- Gateway/Chat UI: sanear o texto da carga final não-streaming `chat.send`/`chat.inject` com a mesma remoção de envelope/contexto não confiável usada por `chat.history`, prevenindo que o markup wrapper `<<<EXTERNAL_UNTRUSTED_CONTENT...>>>` seja renderizado no chat da UI de Controle. (#24012) Obrigado @mittelaltergouda.
- Telegram/Mídia: enviar uma resposta de Telegram voltada para o usuário quando o download de mídia falha (erros que não sejam de tamanho) em vez de descartar a mensagem silenciosamente.
- Telegram/Webhook: manter os monitores de webhook vivos até que os sinais de aborto do gateway sejam disparados, prevenindo saídas falsas de canais e loops imediatos de reinicialização automática de webhook.
- Telegram/Polling: tentar novamente falhas de rede recuperáveis em tempo de configuração na inicialização do monitor e aguardar o desligamento do executor antes da tentativa para evitar sessões de polling sobrepostas.
- Telegram/Polling: limpar os webhooks do Telegram (`deleteWebhook`) antes de iniciar o `getUpdates` por polling longo, incluindo o tratamento de tentativas para falhas de limpeza transitórias.
- Telegram/Webhook: adicionado suporte de configuração `channels.telegram.webhookPort` e passagem através da fiação de inicialização do plugin para o ouvinte do monitor.
- Browser/Extension Relay: refatoração do worker MV3 para preservar anexos de depurador através de quedas de relay, auto-reconexão com backoff+jitter limitado, persistência e reidratação do estado da aba anexada via `chrome.storage.session`, recuperação de desanexações de navegação `target_closed`, proteção de manipuladores de socket obsoletos, imposição de bloqueios de operação por aba e timeouts por requisição, e adição de hooks de keepalive/atualização de badge de ciclo de vida (`alarms`, `webNavigation`). (#15099, #6175, #8468, #9807)
- Browser/Relay: tratamento do websocket de extensão como conectado apenas quando estiver `OPEN`, permissão de reconexão quando um socket de extensão obsoleto `CLOSING/CLOSED` permanece, e proteção de manipuladores de mensagem/fechamento de socket obsoletos para que eventos tardios não possam limpar o estado de relay ativo; inclui cobertura de regressão para rejeição duplicada ao vivo `409` e corridas de reconexão imediata após o fechamento. (#15099, #18698, #20688)
- Browser/Remote CDP: extensão da recuperação de alvo obsoleto para que `ensureTabAvailable()` agora reutilize a única aba disponível para perfis CDP remotos (mesmo comportamento que os perfis de extensão) enquanto preserva erros estritos de `tab not found` quando existem várias abas; inclui testes de regressão de perfil remoto. (#15989)
- Gateway/Pairing: tratar `operator.admin` como satisfazendo outras verificações de escopo `operator.*` durante a verificação de autenticação de dispositivo para que as sessões locais da CLI/TUI parem de entrar em loops de pareamento obrigatório para comandos com escopo de pareamento/aprovação. (#22062, #22193, #21191) Obrigado @Botaccess, @jhartshorn e @ctbritt.
- Gateway/Pairing: auto-aprovar requisições de pareamento `scope-upgrade` de loopback (incluindo reconexões de token de dispositivo) para que os clientes locais não desconectem na elevação de escopo obrigatória de pareamento. (#23708) Obrigado @widingmarcus-cyber.
- Gateway/Scopes: incluir `operator.read` e `operator.write` nos pacotes de escopo de conexão de operador padrão nas instâncias da CLI, Control UI e macOS para que as chamadas subsequentes de anúncio/sub-agente com escopo de escrita não atinjam mais desconexões de "pareamento obrigatório" em gateways de loopback. (#22582) obrigado @YuzuruS.
- Gateway/Pairing: tratamento de tokens de pareamento `operator.admin` como satisfatórios para requisições `operator.write` para que dispositivos legados parem de entrar em loop nos avisos de upgrade de escopo introduzidos na versão 2026.2.19. (#23125, #23006) Obrigado @vignesh07.
- Gateway/Restart: correção de casos de borda de loop de reinicialização mantendo a detecção de bootstrap `openclaw.mjs -> dist/entry.js` explícita, readquirindo o bloqueio do gateway para caminhos de fallback de reinicialização em processo e estreitando a cobertura de regressão de loop de reinicialização. (#23416) Obrigado @jeffwnli.
- Gateway/Lock: uso da acessibilidade opcional da porta do gateway como um sinal primário de vivacidade de bloqueio obsoleto (e vinculação da aquisição do bloqueio do loop de execução do gateway à porta resolvida), reduzindo bloqueios falsos de "já está em execução" após saídas não limpas. (#23760) Obrigado @Operative-001.
- Entrega/Fila: quarentena de entradas da fila imediatamente em erros de entrega permanentes conhecidos (por exemplo, destinatários inválidos ou referências de conversa ausentes), movendo-as para `failed/` em vez de tentar novamente em cada reinicialização. (#23794) Obrigado @aldoeliacim.
- Cron/Status: divisão do resultado da execução (`lastRunStatus`) do resultado da entrega (`lastDeliveryStatus`) no estado de cron persistido, eventos finalizados e histórico de execução para que a entrega de anúncios falhos/desconhecidos seja visível sem confundi-la com erros de execução.
- Cron/Entrega: roteamento de jobs de anúncio apenas de texto com alvos explícitos de thread/tópico através de entrega direta de saída para que os destinos de fórum/thread não sejam descartados por turnos de anúncio intermediários. (#23841) Obrigado @AndrewArto.
- Cron: respeitar `cron.maxConcurrentRuns` no loop do temporizador para que os jobs devidos possam ser executados até o paralelismo configurado em vez de sempre serem executados serialmente. (#11595) Obrigado @Takhoffman.
- Cron/Execução: imposição da mesma proteção de timeout por job para execuções manuais de `cron.run` que as execuções orientadas por temporizador, incluindo propagação de aborto para jobs de agentes isolados, para que as execuções forçadas não possam travar indefinidamente. (#23704) Obrigado @tkuehnl.
- Cron/Execução: persistência do marcador `runningAtMs` da execução manual antes de liberar o bloqueio do cron para que tiques de temporizador sobrepostos não possam iniciar o mesmo job simultaneamente.
- Cron/Inicialização: imposição de proteções de timeout por job para execuções de replay de recuperação na inicialização para que jobs isolados perdidos não possam travar indefinidamente durante a recuperação de boot do gateway.
- Cron/Sessão principal: respeitar os sinais de aborto/timeout ao tentar novamente loops de contenção de heartbeat `wakeMode=now` para que as execuções de cron de alvo principal parem prontamente em vez de esperar por toda a janela de re-tentativa ocupada.
- Cron/Agendamento: para jobs `every`, preferência por `lastRunAtMs + everyMs` quando ainda no futuro após reinicializações, e depois recorrer ao agendamento âncora para janelas de recuperação, para que o PRÓXIMO tempo corresponda à última cadência bem-sucedida. (#22895) Obrigado @SidQin-cyber.
- Cron/Serviço: execução de jobs manuais de `cron.run` fora do bloqueio do cron (enquanto ainda persiste o estado iniciado/finalizado atomicamente) para que o `cron.list` e `cron.status` permaneçam responsivos durante execuções forçadas longas. (#23628) Obrigado @dsgraves.
- Cron/Temporizador: manutenção de um temporizador de re-verificação watchdog armado enquanto o `onTimer` está sendo executado ativamente para que o agendador continue o polling mesmo que um tique de execução devida pare por um período prolongado. (#23628) Obrigado @dsgraves.
- Cron/Log de execução: limpeza das entradas da fila de escrita do log de execução por caminho liquidadas para que o tempo de atividade do cron de longa duração não retenha a manutenção de promessas obsoletas na memória.
- Cron/Log de execução: endurecimento da resolução do caminho do log de execução de `cron.runs` rejeitando entradas de `id`/`jobId` com separador de caminho e impondo leituras dentro do diretório `runs/` por cron.
- Cron/Anunciar: quando a resolução do alvo de entrega do anúncio falha (por exemplo, vários canais configurados sem alvo explícito), pulada a injeção de fallback `Cron (error): ...` na sessão principal para que as execuções falhem de forma limpa sem envios acidentais na última rota. (#24074)
- Cron/Telegram: validação do `delivery.to` do cron com a análise de alvo do Telegram compartilhada e resolução de alvos legados `@username`/`t.me` para IDs numéricos no momento do envio para gravação determinística do alvo de entrega. (#21930) Obrigado @kesor.
- Telegram/Alvos: normalização dos alvos qualificados por tópico sem prefixo através do caminho compartilhado de análise/normalização para que as rotas `@channel:topic:<id>` e `<chatId>:topic:<id>` válidas sejam reconhecidas novamente. (#24166) Obrigado @obviyus.
- Cron/Isolamento: forçar IDs de sessão novos para execuções isoladas de cron para que as execuções `sessionTarget="isolated"` nunca reutilizem o contexto de execuções anteriores. (#23470) Obrigado @echoVic.
- Plugins/Instalações: remoção de entradas de `devDependency` do tipo `workspace:*` dos manifestos de plugins copiados antes do `npm install --omit=dev`, prevenindo falhas de instalação `EUNSUPPORTEDPROTOCOL` para plugins de canal publicados no npm (incluindo Feishu e MS Teams).
- Feishu/Plugins: restaurada a disponibilidade do SDK do Feishu agrupado para instalações globais e remoção de `openclaw: workspace:*` das `devDependencies` do plugin durante a sincronização da versão do plugin para que os plugins instalados via npm não falhem na instalação de dependências. (#23611, #23645, #23603)
- Config/Canais: auto-ativação de canais integrados gravando `channels.<id>.enabled=true` (não `plugins.entries.<id>`) e interrupção da adição de integrados ao `plugins.allow`, prevenindo falhas de validação `plugins.entries.telegram: plugin not found`.
- Config/Canais: quando o `plugins.allow` está ativo, os fluxos de auto-ativação/ativação agora também permitem canais integrados configurados para que `channels.<id>.enabled=true` não permaneça bloqueado por listas de permissão de plugins restritivas.
- Plugins/Descoberta: ignorar padrões de diretório de extensão backup/desativado (por exemplo, `.backup-*`, `.bak`, `.disabled*`) e mover os diretórios de backup do atualizador para `.openclaw-install-backups`, prevenindo colisões de ID de plugin duplicadas de cópias arquivadas.
- Plugins/CLI: fazer com que os fluxos de `openclaw plugins enable` e instalação/link de plugin atualizem as listas de permissões via política compartilhada de ativação de plugin para que os plugins ativados não fiquem desativados por incompatibilidade de lista de permissões. (#23190) Obrigado @downwind7clawd-ctrl.
- Segurança/Chamada de voz: endurecimento do tratamento de WebSocket do fluxo de mídia contra DoS de conexão ociosa pré-autenticação, adicionando timeouts estritos pré-início, limites de conexão pendentes por IP e limites totais de conexão para os endpoints de streaming. Obrigado @jiseoung por reportar.
- Segurança/Sessões: redação de padrões de token sensíveis da saída da ferramenta `sessions_history` e exibição de metadados `contentRedacted` quando a ocultação ocorre. (#16928) Obrigado @aether-ai-agent.
- Segurança/Execução: interrupção da confiança em diretórios derivados do `PATH` para verificações de lista de permissões de binários seguros, adição de `tools.exec.safeBinTrustedDirs` explícitos e fixação da execução de shell de binários seguros aos caminhos executáveis absolutos resolvidos para evitar desvios de aprovação por sombreamento de binários. Obrigado @tdjackey por reportar.
- Segurança/Elevado: correspondência do `tools.elevated.allowFrom` apenas contra identidades de remetente (não destinatário `ctx.To`), fechando um desvio de token de destinatário para autorização `/elevated`. Obrigado @jiseoung por reportar.
- Segurança/Feishu: imposição de correspondência de lista de permissão apenas por ID para autorização de remetente de DM/grupo, normalização dos prefixos de ID do Feishu durante as verificações e ignorar nomes de exibição mutáveis para que colisões de nomes de exibição não satisfaçam as entradas da lista de permissões. Obrigado @jiseoung por reportar.
- Segurança/Política de grupo: endurecimento da correspondência de `channels.*.groups.*.toolsBySender` exigindo tipos de chaves de remetente explícitos (`id:`, `e164:`, `username:`, `name:`), prevenindo colisões entre identificadores em campos mutáveis/nomes de exibição, mantendo chaves sem tipo legadas em um caminho apenas por ID depreciado. Obrigado @jiseoung por reportar.
...
- Node/host exec do macOS: padrão para node `system.run` sem interface gráfica do macOS executado localmente e rota através do app acompanhante apenas quando `OPENCLAW_NODE_EXEC_HOST=app` estiver explicitamente definido, evitando incompatibilidades de namespace do sistema de arquivos do app acompanhante durante a execução. (#23547)
- Sandbox/Media: mapear caminhos de workspace de contêiner (`/workspace/...` e `file:///workspace/...`) de volta para a raiz de sandbox do host para validação de mídia de saída, prevenindo erros de negação falsos para mídia local gerada por sandbox. (#23083) Obrigado @echo931.
- Sandbox/Docker: aplicação de montagens de vinculação personalizadas após as montagens de workspace e priorização da resolução de origem de vinculação em caminhos sobrepostos, para que as vinculações de workspace explícitas não sejam mais ignoradas. (#22669) Obrigado @tasaankaeris.
- Aprovações de Exec/Encaminhamento: restaurado o encaminhamento de texto do Discord quando as aprovações de componentes não estão configuradas, e transporte de capturas instantâneas de solicitação através de eventos de resolução para que avisos resolvidos ainda encaminhem após falhas de cache/reinicializações. (#22988) Obrigado @bubmiller.
- Control UI/WebSocket: parar e limpar o cliente de gateway do navegador no encerramento da UI para que as remontagens não deixem clientes websocket órfãos que criam conexões ativas duplicadas. (#23422) Obrigado @floatinggball-design.
- Control UI/WebSocket: envio de um `instanceId` estável por aba nos quadros de conexão websocket para que os ciclos de reconexão mantenham uma identidade de cliente consistente para diagnósticos e rastreamento de presença. (#23616) Obrigado @zq58855371-ui.
- Config/Memory: permissão de `"mistral"` na validação de esquema de `agents.defaults.memorySearch.provider` e `agents.defaults.memorySearch.fallback`. (#14934) Obrigado @ThomsenDrake.
- Feishu/Comandos: em chats de grupo, a autorização de comando agora recorre ao `channels.feishu.allowFrom` de nível superior quando o `allowFrom` por grupo não está definido, para que o `/command` não seja mais bloqueado por uma lista de permissões vazia não intencional. (#23756)
- Ferramentas de desenvolvimento: prevenção de regressões de alvo de symlink `CLAUDE.md` excluindo sentinelas de symlink CLAUDE do `oxfmt` e marcando-os como `-text` no `.gitattributes`, para que a normalização de formatador/EOL não possa reintroduzir alvos com nova linha à direita. Obrigado @vincentkoc.
- Agentes/Compactação: restaurado o carregamento da extensão de salvaguarda de compactação embutida/poda de contexto em produção conectando as fábricas de extensão agrupadas no carregador de recursos em vez da resolução de caminho de arquivo em tempo de execução. (#22349) Obrigado @Glucksberg.
- Feishu/Mídia: para mensagens de vídeo de entrada que incluem tanto `file_key` (vídeo) quanto `image_key` (miniatura), preferência por `file_key` ao baixar mídia para que os anexos de vídeo sejam salvos em vez de falhar silenciosamente em chaves de miniatura. (#23633)
- Hooks/Loader: evitar recompilação redundante de modulo de hook na reinicialização do gateway pulando o cache-busting para hooks agrupados e usando chaves de metadados de arquivo estáveis (`mtime+size`) para importações de hooks de workspace/gerenciados/plugins mutáveis. (#16953) Obrigado @mudrii.
- Hooks/Cron: supressão de eventos de sessão principal duplicados para turnos de hook entregues e marcação de saídas antecipadas `SILENT_REPLY_TOKEN` (`NO_REPLY`) como entregues para prevenir poluição do contexto do hook. (#20678) Obrigado @JonathanWorks.
- Provedores/OpenRouter: injeção de `cache_control` em prompts de sistema para modelos Anthropic do OpenRouter para melhorar o reuso de prompt-cache. (#17473) Obrigado @rrenamed.
- Instalador/Testes de fumaça: remoção das substituições legadas de `OPENCLAW_USE_GUM` das execuções de docker install-smoke para que os testes exercitem diretamente o comportamento de detecção automática de TTY do instalador.
- Provedores/OpenRouter: permissão de IDs de modelos de pass-through OpenRouter e Opencode na filtragem de modelos ao vivo para que IDs de modelos roteados personalizados sejam tratados como refs modernos. (#14312) Obrigado @Joly0.
- Provedores/OpenRouter: padrão de raciocínio habilitado quando o modelo selecionado anuncia `reasoning: true` e nenhuma substituição de sessão/diretiva está definida. (#22513) Obrigado @zwffff.
- Provedores/OpenRouter: mapeamento de níveis de `/think` para `reasoning.effort` em execuções incorporadas, preservando as cargas úteis explícitas de `reasoning.max_tokens`. (#17236) Obrigado @robbyczgw-cla.
- Provedores/OpenRouter: preservação do provedor de sessão armazenado quando os IDs de modelo têm prefixo de fornecedor (por exemplo, `anthropic/...`) para que os turnos subsequentes não roteiem incorretamente para APIs de provedor diretas. (#22753) Obrigado @dndodson.
- Provedores/OpenRouter: preservação do prefixo `openrouter/` obrigatório para IDs de modelos nativos do OpenRouter durante a normalização de ref do modelo. (#12942) Obrigado @omair445.
- Provedores/OpenRouter: passagem de parâmetros de roteamento de provedor de `params.provider` do modelo para as cargas úteis de requisição do OpenRouter para controles de seleção de provedor. (#17148) Obrigado @carrotRakko.
- Provedores/OpenRouter: preservação das entradas de lista de permissões de modelos contendo caminhos predefinidos do OpenRouter (por exemplo, `openrouter/@preset/...`) tratando a análise de perfil de autenticação `/model ...@profile` como uma substituição apenas de sufixo. (#14120) Obrigado @NotMainstream.
- Cron/Auth: propagar a resolução do perfil de autenticação para sessões de cron isoladas para que as chaves de API do provedor sejam resolvidas da mesma forma que as sessões principais, corrigindo erros 401 ao usar provedores configurados via perfis de autenticação. (#20689) Obrigado @lailoo.
- Cron/Acompanhamento: passar o `agentDir` resolvido através de cron isolado e execuções incorporadas de acompanhamento enfileiradas para que as pesquisas de autenticação/perfil permaneçam com escopo no diretório do agente correto. (#22845) Obrigado @seilk.
- Agentes/Mídia: rotear a extração `MEDIA:` do resultado da ferramenta através da validação do analisador compartilhado para que prosa malformada como `MEDIA:-prefixed ...` não seja mais tratada como um caminho de arquivo local (previne substituições de erro de ferramenta Telegram ENOENT). (#18780) Obrigado @HOYALIM.
- Registro: limitar o tamanho de um único arquivo de log com `logging.maxFileBytes` (padrão 500 MB) e suprimir gravações adicionais após atingir o limite para evitar o esgotamento do disco por tempestades de erros repetidas.
- Memória/HTTP Remoto: centralizar as chamadas HTTP de memória remota por trás de um auxiliar protegido compartilhado (`withRemoteHttpResponse`) para que as incorporações e fluxos em lote usem um único caminho de requisição/liberação.
- Memória/Incorporações: aplicar fixação de host de base remota configurada (`allowedHostnames`) em requisições de incorporação OpenAI/Voyage/Gemini para manter os endpoints privados/auto-hospedados funcionando sem desvios entre hosts. (#18198) Obrigado @ianpcook.
- Memória/Lote: rotear solicitações de upload/criação/status/download em lote OpenAI/Voyage/Gemini através do mesmo caminho HTTP protegido para aplicação consistente da política SSRF.
- Memória/Índice: detectar mudanças no conjunto de fontes de memória (por exemplo, habilitar `sessions` após um índice existente apenas de memória) e disparar um reíndice completo para que as transcrições de sessões existentes sejam indexadas sem exigir `--force`. (#17576) Obrigado @TarsAI-Agent.
- Memória/Incorporações: impor um limite de segurança de 8k por entrada antes do lote de incorporação e aplicar um limite de fallback conservador de 2k para provedores locais sem limites de entrada declarados, evitando que blocos de sessão/memória superdimensionados disparem falhas de tamanho de contexto do provedor durante a sincronização/indexação. (#6016) Obrigado @batumilove.
- Memória/QMD: no Windows, resolver nomes de comandos `qmd`/`mcporter` puros para executáveis npm shim (`.cmd`) antes de gerar, para que as atualizações de boot do qmd e as pesquisas baseadas no mcporter não falhem mais com `spawn ... ENOENT` em instalações npm padrão. (#23899) Obrigado @arcbuilder-ai.
- Memória/QMD: analisar a saída de texto simples de `qmd collection list --json` quando versões mais antigas do qmd ignoram o modo JSON, e tentar novamente as pesquisas de memória uma vez após garantir novamente as coleções gerenciadas quando o qmd retorna `Collection not found ...`. (#23613) Obrigado @leozhucn.
- iOS/Watch: normalizar cargas de notificação de ação rápida do watch, oferecer suporte a ações indexadas espelhadas além da primária/secundária e corrigir bloqueadores de assinatura/compilação de alvo de teste de iOS para cobertura de notificação do watch. (#23636) Obrigado @mbelinky.
- Signal/RPC: proteger respostas JSON do Signal RPC malformadas com um erro claro de escopo de status e adicionar cobertura de regressão para respostas JSON inválidas. (#22995) Obrigado @adhitShet.
- Gateway/Subagentes: proteger caminhos de corte de mensagem/chave de sessão de gateway e subagente contra entradas indefinidas para evitar falhas precoces de `Cannot read properties of undefined (reading 'trim')` durante fluxos de spawn e espera de subagente.
- Agentes/Workspace: proteger `resolveUserPath` contra entrada indefinida/nula para evitar falhas `Cannot read properties of undefined (reading 'trim')` quando os caminhos do workspace estão ausentes nos fluxos de execução incorporados.
- Autenticação/Perfis: manter as janelas ativas de `cooldownUntil`/`disabledUntil` imutáveis entre as tentativas para que as falhas no meio da janela não possam estender a recuperação indefinidamente; apenas recalcular uma janela de backoff após o prazo anterior ter expirado. Isso resolve loops de re-tentativa de cron/entrada que poderiam prender os gateways até a limpeza manual de `usageStats`. (#23516, #23536) Obrigado @arosstale.
- Canais/Segurança: falha por padrão na configuração de política de grupo de provedor ausente, definindo a política de grupo de tempo de execução como `allowlist` (em vez de herdar `channels.defaults.groupPolicy`) quando `channels.<provider>` está ausente nos canais de mensagem, e alinhar avisos/documentos de tempo de execução + segurança ao mesmo comportamento de fallback (fluxos de usuários Slack, Discord, iMessage, Telegram, WhatsApp, Signal, LINE, Matrix, Mattermost, Google Chat, IRC, Nextcloud Talk, Feishu e Zalo; além de caminhos de mensagem/comando nativo do Discord). (#23367) Obrigado @bmendonca3.
- Gateway/Onboarding: endurecimento dos padrões e orientações de onboarding de gateway remoto, definindo URLs diretas descobertas como `wss://`, rejeitando alvos `ws://` não-loopback inseguros na validação de onboarding e expandindo as mensagens de remediação de segurança remota nos fluxos de cliente/chamada/doctor do gateway. (#23476) Obrigado @bmendonca3.
- CLI/Sessões: passar o diretório de sessões configurado ao resolver os caminhos de transcrição no `agentCommand`, para que os locais de `session.store` personalizados retomem as sessões de forma confiável. Obrigado @davidrudduck.
- Signal/Monitor: tratar os desligamentos de aborto iniciados pelo usuário como saídas limpas quando o `signal-cli` iniciado automaticamente é encerrado, enquanto ainda exibe saídas inesperadas do daemon como falhas de inicialização/tempo de execução. (#23379) Obrigado @frankekn.
- Canais/Dedupe: centralização das primitivas de deduplicação de plugin no SDK do plugin (memória + persistente), movimentação da deduplicação de entrada do Feishu para um armazenamento persistente com escopo de namespace e reuso da lógica de cache de deduplicação compartilhada para replay de webhook do Zalo + rastreamento de mensagens processadas do Tlon para reduzir o tratamento duplicado durante os caminhos de reconexão/replay. (#23377) Obrigado @SidQin-cyber.
- Canais/Entrega: remoção de fallbacks de entrega do WhatsApp codificados; requer contexto de canal explícito/sessão ou escolha automática do único canal configurado quando não houver ambiguidade. (#23357) Obrigado @lbo728.
- ACP/Gateway: aguardar o hello do gateway antes de abrir solicitações de ACP e falhar rapidamente em falhas de conexão pré-hello para evitar travamentos na inicialização e corridas de solicitação "gateway não conectado". (#23390) Obrigado @janckerchen.
- Gateway/Auth: preservação da precedência da substituição de env `OPENCLAW_GATEWAY_PASSWORD` para credenciais de chamada de gateway remoto após refatorações do resolvedor compartilhado, prevenindo que senhas remotas configuradas obsoletas substituam a rotação de segredos em tempo de execução.
- Gateway/Auth: preservação de erros de autenticação `gateway token mismatch` de token compartilhado quando as verificações de token de dispositivo de fallback de `auth.token` falham, e reserva de orientação `device token mismatch` para falhas explícitas de `auth.deviceToken`.
- Gateway/Tools: quando as ferramentas do agente passam uma substituição de `gatewayUrl` na lista de permissões, resolver os tokens de substituição locais a partir do fallback de env/config, mas manter as substituições remotas estritas a `gateway.remote.token`, evitando o vazamento de tokens locais para alvos remotos.
- Gateway/Client: manter os tokens de autenticação de dispositivo em cache em fechamentos de `device token mismatch` quando o cliente usou credenciais explícitas de token/senha compartilhados, evitando a rotatividade acidental de tokens de pareamento durante falhas de autenticação explícitas.
- Node host/Exec: manter o comportamento estrito da lista de permissões do Windows para execuções de wrapper de shell `cmd.exe /c` e retornar orientação de aprovação explícita quando bloqueado (`SYSTEM_RUN_DENIED: allowlist miss`).
- Control UI: exibir orientação de pareamento obrigatório (comandos + lembrete de URL tokenizada móvel) quando o painel desconecta com `1008 pairing required`.
- Segurança/Auditoria: adição da detecção `openclaw security audit` para políticas de grupo abertas que expõem ferramentas de tempo de execução/sistema de arquivos sem proteções de sandbox/workspace (`security.exposure.open_groups_with_runtime_or_fs`).
- Segurança/Auditoria: tornar a gravidade de `gateway.real_ip_fallback_enabled` condicional para configurações de proxy confiável de loopback (aviso para `trustedProxies` apenas de loopback, crítico quando proxies não-loopback são confiáveis). (#23428) Obrigado @bmendonca3.
- Segurança/Exec env: bloqueio das substituições de `HOME` e `ZDOTDIR` com escopo de requisição nos saneadores de ambiente de execução do host (Node + macOS), prevenindo a execução do arquivo de inicialização do shell antes dos corpos de comando avaliados pela lista de permissões. Obrigado @tdjackey por reportar.
- Segurança/Exec env: bloqueio de `SHELLOPTS`/`PS4` nos saneadores de ambiente de execução do host e restrição das substituições de ambiente de requisição do wrapper de shell (`bash|sh|zsh ... -c/-lc`) a uma pequena lista de permissões explícita (`TERM`, `LANG`, `LC_*`, `COLORTERM`, `NO_COLOR`, `FORCE_COLOR`) nos caminhos do node host e do acompanhante macOS, prevenindo desvios da lista de permissões por substituição de comando de prompt xtrace. Obrigado @tdjackey por reportar.
- WhatsApp/Security: impor `allowFrom` para alvos de saída de mensagem direta em todos os modos de envio (incluindo `mode: "explicit"`), prevenindo envios para números que não estão na lista de permissões. (#20108) Obrigado @zahlmann.
- Segurança/Aprovações de Exec: falha por padrão em continuações de linha de shell (`\\\n`/`\\\r\n`) e tratamento da execução do shell-wrapper como aprovação obrigatória no modo lista de permissões, prevenindo desvios de substituição de comando de nova linha `$\\`. Obrigado @tdjackey por reportar.
- Segurança/Gateway: exibir um aviso de segurança na inicialização quando flags de configuração inseguras/perigosas estão habilitadas (incluindo `gateway.controlUi.dangerouslyDisableDeviceAuth=true`) e apontar os operadores para `openclaw security audit`.
- Segurança/Hooks auth: normalizar as chaves de IP do cliente de limite de taxa de autenticação de hook para que endereços IPv4 e IPv6 mapeados para IPv4 compartilhem um único balde de controle, prevenindo desvios de orçamento de tentativa de autenticação de formulário duplo. Obrigado @aether-ai-agent por reportar.
- Segurança/Aprovações de Exec: tratar os wrappers de despacho de shell e `env` como transparentes durante a análise da lista de permissões nos caminhos do node-host e do acompanhante macOS para que as verificações de política correspondam ao executável efetivo/carga útil de shell inline em vez do binário wrapper, bloqueando desvios de lista de permissões contrabandeados pelo wrapper. Obrigado @tdjackey por reportar.
- Segurança/Aprovações de Exec: exigir perfis de binários seguros explícitos para entradas de `tools.exec.safeBins` no modo lista de permissões (remover o fallback de perfil de binário seguro genérico) e adicionar `tools.exec.safeBinProfiles` para binários personalizados seguros para que entradas de estilo de intérprete sem perfil não possam ser tratadas como seguras para stdin. Obrigado @tdjackey por reportar.
- Segurança/Canais: endurecer o tratamento de tokens de menu externo do Slack mudando para tokens CSPRNG, validando o formato do token, exigindo identidade do usuário para pesquisas de opções externas e evitando fallbacks de `trigger_id` de carimbo de data/hora fabricados; também mudar IDs de canal Tlon Urbit para UUIDs CSPRNG, centralizar a geração segura de ID/token via ajudantes de infraestrutura compartilhados e adicionar um teste de proteção para bloquear novos padrões de token/id `Date.now()+Math.random()` em tempo de execução.
- Segurança/Hooks transforms: impor a contenção segura de symlink para caminhos de módulo de transformação de webhook (incluindo `hooks.transformsDir` e `hooks.mappings[].transform.module`) resolvendo ancestrais de caminho existentes via realpath antes da importação, preservando o suporte a symlink dentro da raiz; adicionar cobertura de regressão para casos de escape e permissão. Obrigado @aether-ai-agent por reportar.
- Telegram/WSL2: desativar `autoSelectFamily` por padrão no WSL2 e memorizar a detecção do WSL2 na lógica de decision de rede do Telegram para evitar repetidas sondagens de `/proc/version` sincronizadas nos caminhos de busca/envio. (#21916) Obrigado @MizukiMachine.
- Telegram/Rede: fixar a ordenação de resultados de DNS do Node 22+ como `ipv4first` para caminhos de busca do Telegram e adicionar substituições `OPENCLAW_TELEGRAM_DNS_RESULT_ORDER`/`channels.telegram.network.dnsResultOrder` para reduzir falhas de busca no caminho IPv6. (#5405) Obrigado @Glucksberg.
- Telegram/Rajadas de encaminhamento: agrupar atualizações de texto+mídia encaminhadas através de uma janela de debounce de faixa de encaminhamento dedicada que funciona com a configuração de debounce de entrada padrão, mantendo os comandos de controle encaminhados imediatos. (#19476) obrigado @napetrov.
- Telegram/Streaming: preservar o mapeamento de visualização de rascunho arquivado após o flush e limpar bolhas de visualização de raciocínio substituídas para que as finais de visualização de várias mensagens não editem cruzado ou deixem mensagens obsoletas órfãs sob corridas de envio/rotação. (#23202) Obrigado @obviyus.
- Telegram/Respostas: restringir a deduplicação de texto/mídia da ferramenta de mensagens apenas para envios para o mesmo alvo, para que os envios de ferramentas para alvos cruzados não possam mais suprimir silenciosamente as respostas finais do Telegram.
- Telegram/Respostas: normalizar variantes de `file://` e caminho de mídia local durante a deduplicação de mensagens para que caminhos de mídia equivalentes não produzam respostas do Telegram duplicadas.
- Telegram/Respostas: extrair o contexto de origem encaminhado de alvos de resposta unificados (`reply_to_message` e `external_reply`) para que os metadados de encaminhamento+comentário sejam preservados em formatos de resposta parcial. (#9720) obrigado @mcaxtr.
- Telegram/Polling: persistir uma marca d'água de offset de atualização segura limitada por atualizações pendentes para que travamentos/reinicializações não possam pular atualizações de `update_id` inferiores enfileiradas após a conclusão fora de ordem. (#23284) obrigado @frankekn.
- Telegram/Polling: forçar a reinicialização de instâncias de executor presas quando as rejeições de rede não tratadas recuperáveis escapam do caminho da tarefa de polling, para que o polling seja retomado em vez de parar silenciosamente. (#19721) Obrigado @jg-noncelogic.
- Slack/Comandos Slash: preservar o receptor do app Bolt ao registrar manipuladores de opções de seleção externa para que a inicialização do monitor não falhe em tempos de execução que exigem chamadas de `app.options` vinculadas. (#23209) Obrigado @0xgaia.
- Sessões de comandos slash Slack/Telegram: aguardar a persistência dos metadados da sessão antes do despacho para que as execuções de comandos slash nativos de primeiro turno não concorram com as atualizações de metadados de origem da sessão. (#23065) obrigado @hydro13.
- Slack/Roteamento de fila: preservar valores de `thread_ts` tipo string através do esvaziamento da fila em modo de coleta e atualizações de `deliveryContext` de DM para que os acompanhamentos encadeados (threaded) não vazem para o canal principal quando os IDs de thread do Slack são strings. (#11934) Obrigado @sandieman2 e @vincentkoc.
- Telegram/Comandos Nativos: definir `ctx.Provider="telegram"` para o contexto do comando slash nativo para que as verificações de portal elevado resolvam o provedor corretamente (corrige falhas de `provider (ctx.Provider)` em fluxos `/elevated`). (#23748) Obrigado @serhii12.
- Agentes/Ollama: preservar argumentos de chamada de ferramenta de inteiros inseguros como strings exatas durante a análise NDJSON, prevenindo que IDs numéricos grandes sejam arredondados antes da execução da ferramenta. (#23170) Obrigado @BestJoester.
- Cron/Gateway: manter o `cron.list` e `cron.status` responsivos durante a recuperação de inicialização, evitando um bloqueio de cron mantido por muito tempo enquanto os jobs perdidos são executados. (#23106) Obrigado @jayleekr.
- Gateway/Recarregamento de Config: comparar caminhos de configuração com valor de array estruturalmente durante a diferenciação para que `memory.qmd.paths` e `memory.qmd.scope.rules` inalterados não disparem mais recarregamentos falsos de reinicialização obrigatória. (#23185) Obrigado @rex05ai.
- Gateway/Recarregamento de Config: tentar novamente capturas instantâneas de configuração ausentes de curta duração durante o recarregamento antes de pular, prevenindo janelas de unlink de escrita atômica de disparar loops de reinicialização. (#23343) Obrigado @lbo728.
- Cron/Agendamento: validar expressões cron de tempo de execução antes da avaliação de agendamento/escalonamento para que jobs persistidos malformados reportem um erro claro `invalid cron schedule: expr is required` em vez de falhar com erros `undefined.trim` e rotatividade de auto-desativação. (#23223) Obrigado @asimons81.
- Memória/QMD: migrar associações de coleção sem escopo legadas (por exemplo, `memory-root`) para nomes com escopo por agente (por exemplo, `memory-root-main`) durante a inicialização quando seguro, para que a `memory_search` baseada em QMD não falhe mais com `Collection not found` após as atualizações. (#23228, #20727) Obrigado @JLDynamics e @AaronFaby.
- Memória/QMD: normalizar as consultas de busca BM25 do script Han antes de invocar o `qmd search` para que prompts mistos CJK+Latim não retornem mais resultados vazios devido à incompatibilidade do tokenizador. (#23426) Obrigado @LunaLee0130.
- TUI/Entrada: habilitar o agrupamento de rajadas de colagem multilinhas no Terminal.app do macOS e iTerm para que os blocos colados não sejam mais enviados linha por linha como mensagens separadas. (#18809) Obrigado @fwends.
- TUI/RTL: isolar linhas de script da direita para a esquerda (intervalos de Árabe/Hebraico) com marcas de isolamento bidi Unicode na sanitização de texto da TUI para que a saída do assistente RTL não seja mais renderizada em ordem visual invertida nos painéis de chat do terminal. (#21936) Obrigado @Asm3r96.
- TUI/Status: solicitar renderizações imediatas após definir os estados de atividade `sending`/`waiting` para que as execuções em andamento sempre mostrem indicadores de progresso visíveis em vez de parecerem ociosas até a conclusão. (#21549) Obrigado @13Guinness.
- TUI/Entrada: armar o timing de saída com Ctrl+C ao limpar o texto do compositor não vazio e adicionar um caminho de fallback para SIGINT para que as saídas com Ctrl+C duplo permaneçam responsivas durante as execuções ativas em vez de exigir um toque extra ou parecer travado. (#23407) Obrigado @tinybluedev.
- Agentes/Fallbacks: tratar cargas úteis JSON com `type: "api_error"` + `"Internal server error"` como erros de failover transitórios para que falhas no estilo Anthropic 500 disparem o fallback do modelo. (#23193) Obrigado @jarvis-lane.
- Agentes/Google: sanear valores `thought_signature`/`thoughtSignature` que não sejam base64 das transcrições de replay do assistente para requisições nativas do Google Gemini, preservando assinaturas válidas e a ordem das chamadas de ferramentas. (#23457) Obrigado @echoVic.
- Agentes/Transcrições: validar os nomes das chamadas de ferramentas do assistente (sintaxe/comprimento + lista de permissões de ferramentas registradas) antes da persistência da transcrição e durante a sanitização do replay para que nomes de ferramentas de failover malformados não envenenem mais as sessões com erros repetidos HTTP 400 do provedor. (#23324) Obrigado @johnsantry.
