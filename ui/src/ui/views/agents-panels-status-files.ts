...
export function renderAgentChannels(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t("agents.channelsPanel.contextSubtitle"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            <div class="card-title">${t("agents.channelsPanel.title")}</div>
            <div class="card-sub">${t("agents.channelsPanel.subtitle")}</div>
          </div>
        ...
        <div class="muted" style="margin-top: 8px;">
          ${t("agents.channelsPanel.lastRefresh", { time: lastSuccessLabel })}
        </div>
        ${
          params.error
        ...
        ${
          !params.snapshot
            ? html`
                <div class="callout info" style="margin-top: 12px">${t("agents.channelsPanel.loadHint")}</div>
              `
            : nothing
        }
        ${
          entries.length === 0
            ? html`
                <div class="muted" style="margin-top: 16px">${t("agents.channelsPanel.noChannels")}</div>
              `
            : html`
                <div class="list" style="margin-top: 16px;">
                  ${entries.map((entry) => {
                    ...
                    const status = summary.total
                      ? t("agents.channelsPanel.connectedCount", {
                          connected: String(summary.connected),
                          total: String(summary.total),
                        })
                      : t("agents.channelsPanel.noAccounts");
                    const config = summary.configured
                      ? t("agents.channelsPanel.configuredCount", {
                          configured: String(summary.configured),
                        })
                      : t("agents.channelsPanel.notConfigured");
                    const enabled = summary.total
                      ? t("agents.channelsPanel.enabledCount", { enabled: String(summary.enabled) })
                      : t("common.disabled");
...
export function renderAgentCron(params: {
  ...
  return html`
    <section class="grid grid-cols-2">
      ${renderAgentContextCard(params.context, t("agents.cronPanel.contextSubtitle"))}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
    ...
    <section class="card">
      ...
      ${
        jobs.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agents.cronPanel.noJobs")}</div>
            `
          : html`
              <div class="list" style="margin-top: 16px;">
                ${jobs.map(
...
export function renderAgentFiles(params: {
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("agents.filesPanel.title")}</div>
          <div class="card-sub">${t("agents.filesPanel.subtitle")}</div>
        </div>
        <button
          class="btn btn--sm"
      ...
      ${
        list
          ? html`<div class="muted mono" style="margin-top: 8px;">${t("agents.filesPanel.workspace", { workspace: list.workspace })}</div>`
          : nothing
      }
      ${
              ...
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${
                    files.length === 0
                      ? html`
                          <div class="muted">${t("agents.filesPanel.noFiles")}</div>
                        `
                      : files.map((file) =>
                ...
                <div class="agent-files-editor">
                  ${
                    !activeEntry
                      ? html`
                          <div class="muted">${t("agents.filesPanel.selectFile")}</div>
                        `
                      : html`
                            ...
                            activeEntry.missing
                              ? html`
                                  <div class="callout info" style="margin-top: 10px">
                                    ${t("agents.filesPanel.missingHint")}
                                  </div>
                                `
                              : nothing
                          }
                          <label class="field" style="margin-top: 12px;">
                            <span>${t("agents.filesPanel.contentLabel")}</span>
                            <textarea
                              .value=${draft}
                              @input=${(e: Event) =>
  ...
  return html`
    ...
    >
      ...
      ${
        file.missing
          ? html`
              <span class="agent-pill warn">${t("agents.filesPanel.missing")}</span>
            `
          : nothing
      }
    </button>
