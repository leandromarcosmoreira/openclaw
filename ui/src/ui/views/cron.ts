...
export function renderCron(props: CronProps) {
  ...
  const selectedRunTitle =
    selectedJob?.name ?? props.runsJobId ?? t("cron.history.selectPlaceholder");
  ...
  return html`
    <section class="grid grid-cols-2">
      <div class="card">
        <div class="card-title">${t("cron.gateway.title")}</div>
        <div class="card-sub">${t("cron.gateway.subtitle")}</div>
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            <div class="stat-label">${t("cron.gateway.jobs")}</div>
            <div class="stat-value">${props.status?.jobs ?? t("common.na")}</div>
          </div>
          <div class="stat">
            <div class="stat-label">${t("cron.gateway.nextWake")}</div>
      ...
      <div class="card">
        <div class="card-title">${t("cron.form.title")}</div>
        <div class="card-sub">${t("cron.form.subtitle")}</div>
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("cron.history.subtitle", { title: selectedRunTitle })}</div>
      ${
        props.runsJobId == null
          ? html`
              <div class="muted" style="margin-top: 12px">${t("cron.history.noSelectedJob")}</div>
            `
          : orderedRuns.length === 0
            ? html`
                <div class="muted" style="margin-top: 12px">${t("cron.history.noRuns")}</div>
              `
            : html`
              <div class="list" style="margin-top: 12px;">
...
function renderScheduleFields(props: CronProps) {
  ...
  if (form.scheduleKind === "every") {
    return html`
      <div class="form-grid" style="margin-top: 12px;">
        <label class="field">
          <span>${t("cron.schedule.every")}</span>
          <input
  ...
  return html`
    <div class=${itemClass} @click=${() => props.onLoadRuns(job.id)}>
      <div class="list-main">
        ...
        ${job.agentId ? html`<div class="muted cron-job-agent">${t("cron.item.agent", { id: job.agentId })}</div>` : nothing}
      </div>
      <div class="list-meta">
        ${renderJobState(job)}
      </div>
    ...
    ${
      delivery
        ? html`<div class="cron-job-detail">
            <span class="cron-job-detail-label">${t("cron.form.delivery")}</span>
  ...
  return html`
    <div class="list-item">
      ...
      <div class="list-meta">
        ...
        ${
          chatUrl
            ? html`<div><a class="session-link" href=${chatUrl}>${t("cron.history.openChat")}</a></div>`
            : nothing
        }
