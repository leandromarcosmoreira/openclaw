...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  const roles = t("nodes.roles", { roles: formatList(device.roles) });
  const scopes = t("nodes.scopes", { scopes: formatList(device.scopes) });
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  ...
  const scopes = t("nodes.scopes", { scopes: formatList(token.scopes) });
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        ...
        <button
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("config.actions.applying") : t("common.save")}
        </button>
      </div>
        ...
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("common.loading") : t("common.reload")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.noNodesWithSystemRun")}</div>
                        `
                      : nothing
                  }
                </div>
        ...
        <div class="list-sub">
          ...
          ${
            bindingValue === "__default__"
              ? t("nodes.usesDefault", { name: state.defaultBinding ?? t("common.any") })
              : t("nodes.override", { node: agent.binding ?? "" })
          }
        </div>
      </div>
      <div class="list-meta">
      ...
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("common.ok") : t("common.offline")}
          </span>
